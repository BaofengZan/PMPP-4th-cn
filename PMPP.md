...
# 9 Parallel histogram 介绍原子操作和私有化

到目前为止，我们介绍的并行计算模式都允许将计算每个输出元素的任务完全分配给或归属于一个线程。因此，这些模式适合于“拥有者计算”规则，即每个线程都可以写入其指定的输出元素，而不必担心来自其他线程的干扰。这一章介绍了并行直方图计算模式，其中每个输出元素可能被任何线程更新。因此，必须在更新输出元素时协调线程之间的操作，避免任何可能破坏最终结果的干扰。在实践中，还有许多其他重要的并行计算模式，其中输出干扰不容易避免。因此，平行直方图算法提供了这些模式中发生的输出干扰的一个例子。我们首先将检查一种使用原子操作来序列化对每个元素的更新的基本方法。这种基本方法简单但不高效，常常导致令人失望的执行速度。然后，我们将介绍一些广泛使用的优化技术，尤其是私有化，以显著提高执行速度，同时保持正确性。这些技术的成本和收益取决于底层硬件以及输入数据的特性。因此，对于开发者来说，理解这些技术的关键思想，并能够根据不同情况推理其适用性是十分重要的。

## 9.1 背景

直方图是一种显示数据集中数据值出现次数或百分比的图表。在最常见的直方图形式中，值区间沿水平轴绘制，每个区间中数据值的计数以从水平轴上升起的矩形或条形的高度表示。例如，直方图可以用来显示短语“programming massively parallel processors”中字母的频率。为了简单起见，我们假设输入的短语全部为小写。通过检查，我们可以看到字母“a”出现了四次，“b”没有出现，“c”出现了一次，等等。我们定义每个值区间为连续的四个字母范围。因此，第一个值区间是“a”到“d”，第二个值区间是“e”到“h”，以此类推。图9.1显示了根据我们对值区间的定义，显示短语“programming massively parallel processors”中字母频率的直方图。

![image-20240726170149030](imgs/image-20240726170149030.png)

直方图为数据集提供了有用的摘要。在我们的示例中，我们可以看到所代表的短语由集中在字母表中间区间的字母组成，而在后面的区间则明显稀疏。直方图的形状有时被称为数据集的特征，并提供了一种快速确定数据集中是否存在显著现象的方法。例如，信用卡账户的购买类别和位置的直方图形状可以用来检测欺诈性使用。当直方图的形状显著偏离常规时，系统会提出潜在关注的警告。

许多应用领域依赖直方图来总结数据集进行数据分析。计算机视觉就是这样一个领域。不同类型的对象图像，如面部与汽车，倾向于呈现不同的形状。例如，可以绘制图像或图像区域的像素亮度值的直方图。在晴朗的一天，天空的直方图可能只有少数几个非常高的条形在亮度谱的高值区间。通过将图像划分为子区域并分析这些子区域的直方图，可以快速识别出可能包含感兴趣对象的图像的有趣子区域。计算图像子区域的直方图的过程是计算机视觉中特征提取的重要方法，其中特征指的是图像中感兴趣的模式。在实践中，每当需要分析大量数据以提炼有趣事件时，直方图很可能被用作基础计算。信用卡欺诈检测和计算机视觉显然符合这种描述。其他有此类需求的应用领域包括语音识别、网站购买推荐和科学数据分析，如天体运动的相关性分析。

![image-20240726170413520](imgs/image-20240726170413520.png)

直方图可以很容易地以顺序方式计算。图9.2显示了一个计算图9.1中定义的直方图的顺序函数。为了简单起见，直方图函数只需要识别小写字母。C代码假设输入数据集以字符数组data的形式出现，直方图将生成到int数组histo中（第01行）。输入数据项的数量在函数参数length中指定。for循环（第02-07行）顺序遍历数组，识别访问位置data[i]中字符的字母索引，将字母索引保存到变量alphabet_position中，并增加与该区间相关联的histo[alphabet_position/4]元素。字母索引的计算依赖于输入字符串基于标准ASCII码表示，其中字母“a”到“z”被编码为连续值，根据它们在字母表中的顺序。

尽管人们可能不知道每个字母的确切编码值，但可以假设一个字母的编码值是“a”的编码值加上该字母与“a”之间的字母位置差异。在输入中，每个字符都以其编码值存储。因此，表达式data[i] - 'a'（第03行）推导出字母的字母位置，其中“a”的字母位置为0。如果位置值大于或等于0且小于26，则数据字符确实是小写字母（第04行）。请记住，我们定义的区间是每个区间包含四个字母。因此，字母的区间索引是其字母位置值除以4。我们使用区间索引来增加适当的histo数组元素（第05行）。

图9.2中的C代码非常简单高效。算法的计算复杂度为O(N)，其中N是输入数据元素的数量。数据数组元素在for循环中顺序访问，因此当它们从系统DRAM中获取时，CPU缓存行被很好地使用。histo数组非常小，可以很好地适应CPU的一级（L1）数据缓存，这确保了对histo元素的快速更新。对于大多数现代CPU，可以预期这段代码的执行速度是内存限制的，即受限于数据元素可以从DRAM进入CPU缓存的速率。

## 9.2 Atomic operations and a basic histogram kernel

直方图计算的最直接方法是启动与数据元素数量相同的线程，并让每个线程处理一个输入元素。每个线程读取其分配的输入元素，并增加相应字符的间隔计数器。图9.3展示了这种并行化策略的一个例子。请注意，多个线程需要更新同一个计数器（m-p），这是一种称为输出干扰的冲突。程序员必须理解竞争条件和原子操作的概念，以便在他们的并行代码中自信地处理这种输出干扰。

在histo数组中对间隔计数器的增加是一个更新，或对内存位置的读-修改-写操作。该操作涉及读取内存位置（读），给原始值加一（修改），并将新值写回内存位置（写）。读-修改-写是协调协作活动常用的操作。

![image-20240726170730382](imgs/image-20240726170730382.png)

例如，当我们与航空公司预订航班时，我们会打开座位图并寻找可用的座位（读取），我们选择一个座位进行预订（修改），这会将座位状态更改为座位图中的不可用。可能会出现以下不良潜在场景：

- 两位客户同时打开同一航班的座位图。
- 两位客户选择了同一个座位，比如说9C。
- 两位客户都将9C座位的状态更改为不可用

经过这一系列操作后，两位客户都认为自己预订了9C座位。我们可以想象，当他们登机时发现其中一位不能坐在预订的座位上，他们会遇到不愉快的情况！不管你信不信，由于航空公司预订软件的缺陷，现实生活中确实会发生这样不愉快的情况。

 另一个例子是，一些商店允许客户在不排队的情况下等待服务。他们要求每位客户从一个自助机中取一个号码。 有一个显示屏显示下一个将被服务的号码。当服务人员有空时，服务人员要求客户出示与号码匹配的票，验证票，并将显示屏上的号码更新为下一个更高的号码。理想情况下，所有客户都将按照他们进入商店的顺序接受服务。一个不良的结果将是两位客户同时在两个自助机上签到，并且都收到相同号码的票。当服务人员呼叫那个号码时，两位客户都期望自己是应该接受服务的人。

在这两个例子中，不良结果都是由一种称为读-修改-写竞争条件的现象引起的，其中两个或多个同时更新操作的结果取决于所涉及操作的相对时间。有些结果是正确的，有些是错误的。图9.4展示了两个线程试图更新我们文本直方图示例中的同一个histo元素时的竞争条件。图9.4中的每一行显示了一个时间段内的活动，时间从上到下推进。

![image-20240726171054945](imgs/image-20240726171054945.png)

图9.4A描述了一种场景，其中线程1在时间段1到3内完成了其读-修改-写序列的所有三个部分，而在时间段4线程2开始其序列之前。每个操作前面的括号中的值显示了被写入目标的值，假设histo[x]的初始值为0。在这种情况下，操作后histo[x]的值是2，这正是人们所期望的。也就是说，两个线程都成功地增加了histo[x]的值。元素值从0开始，在操作完成后变为2。

在图9.4B中，两个线程的读-修改-写序列重叠。注意，线程1在时间段4将新值写入histo[x]。当线程2在时间段3读取histo[x]时，它的值仍然是0。因此，它计算出的新值以及最终写入histo[x]的值是1而不是2。问题是线程2过早地读取了histo[x]，在线程1完成其更新之前。最终结果是，操作后histo[x]的值是1，这是错误的。线程1的更新丢失了。

在并行执行期间，线程可以相对于彼此以任何顺序运行。在我们的示例中，线程2很容易在线程1之前开始其更新序列。

![image-20240726171352602](imgs/image-20240726171352602.png)

图9.5展示了两种这样的情况。在图9.5A中，线程2在线程1开始之前完成了其更新。在图9.5B中，线程1在线程2完成之前开始了其更新。很明显，图9.5A中的序列对histo[x]产生了正确的结果，但图9.5B中的序列产生了错误的结果。

操作的相对时序导致histo[x]的最终值变化，这表明存在竞争条件。我们可以通过消除线程1和线程2操作序列的可能交错来消除这种变化。也就是说，我们希望允许图9.4A和9.5A中显示的时序，同时消除图9.4B和9.5B中显示的可能性。这可以通过使用原子操作来实现。

对内存位置的原子操作是对内存位置执行读-修改-写序列的操作，以这样的方式进行，即没有其他对该位置的读-修改-写序列可以与之重叠。也就是说，操作的读取、修改和写入部分形成一个不可分割的单元，因此得名原子操作。在实践中，原子操作是通过硬件支持来实现的，以锁定对同一位置的其他操作，直到当前操作完成。在我们的示例中，这种支持消除了图9.4B和9.5B中显示的可能性，因为后续线程不能在领先线程完成其更新序列之前开始其更新序列。

重要的是要记住，原子操作并不强制线程之间的任何特定执行顺序。在我们的示例中，图9.4A和9.5A中显示的两种顺序都被原子操作所允许。线程1可以在线程2之前或之后运行。正在执行的规则是，如果两个线程对同一内存位置执行原子操作，则后续线程执行的原子操作不能在领先线程的原子操作完成之前开始。这有效地将对内存位置执行的原子操作序列化。

原子操作通常根据对内存位置进行的修改来命名。在我们的文本直方图示例中，我们正在向内存位置添加一个值，所以原子操作被称为atomic add。其他类型的原子操作包括 subtraction, increment, decrement, minimum, maximum, logical and, and logical or。CUDA内核可以通过函数调用来对内存位置执行原子添加操作：

![image-20240726171703062](imgs/image-20240726171703062.png)

`atomicAdd` 函数是一个内联函数（参见侧边栏“内联函数”），它被编译成硬件原子操作指令。该指令读取由地址参数指向的全局或共享内存中的32位，并把 `val` 加到旧内容上，然后将结果存回同一地址的内存中。该函数返回地址处的旧值。

> 现代处理器通常提供特殊的指令，这些指令要么执行关键功能（如原子操作），要么显著提高性能（如向量指令）。这些指令通常以内联函数的形式向程序员公开，或者简单地称为内联函数。
>
> 从程序员的角度来看，这些是库函数。然而，编译器会以特殊的方式处理它们；每个这样的调用都被翻译成相应的特殊指令。在最终代码中通常没有函数调用，只有与用户代码一致的特殊指令。所有主要的现代编译器，如GNU编译器集合（gcc）、英特尔C编译器和Clang/LLVM C编译器，都支持内联函数。

![image-20240726171858168](imgs/image-20240726171858168.png)

图9.6展示了一个执行并行直方图计算的CUDA内核。该代码与图9.2中的顺序代码有两个关键区别。第一个区别是，输入元素的循环被替换为线程索引计算（第02行）和边界检查（第03行），以将每个输入元素分配给一个线程。第二个区别是，图9.2中的增量表达式：<img src="imgs/image-20240726171947219.png" alt="image-20240726171947219" style="zoom:67%;" />变成了一个atomicAdd()函数调用（第06行）。第一个参数是要更新的位置的地址&(histo[alphabet_position/4])，第二个参数是要加到该位置的值1。这确保了不同线程对任何histo数组元素的同时更新都能得到正确的序列化。

## 9.3 Latency and throughput of atomic operations

在图9.6的内核中使用的原子操作通过将任何同步的更新序列化到一个位置来确保更新的正确性。正如我们所知，序列化大规模并行程序的任何部分都可以大大增加执行时间，并降低程序的执行速度。因此，这样的序列化操作能够占用尽可能少的执行时间是很重要的。

正如我们在第5章“内存架构和数据局部性”中学到的，访问DRAM中数据的延迟可能需要数百个时钟周期。在第4章“计算架构和调度”中，我们了解到GPU使用零周期上下文切换来容忍这种延迟。在第6章“性能考虑”中，我们了解到只要我们有许多线程，它们的内存访问延迟可以相互重叠，执行速度就受到内存系统吞吐量的限制。因此，GPU充分利用DRAM突发、存储体和通道以实现高内存访问吞吐量是重要的。

此时，读者应该清楚，高内存访问吞吐量的关键是有许多同时进行的DRAM访问。不幸的是，当许多原子操作更新相同的内存位置时，这种策略就会失效。在这种情况下，后续线程的读-修改-写序列必须等待领先线程的读-修改-写序列完成后才能开始。如图9.7所示，同一内存位置的原子操作执行是这样的，一次只能进行一个。每个原子操作的持续时间大约是内存加载的延迟（原子操作时间的左部分）加上内存存储的延迟（原子操作时间的右部分）。每个读-修改-写操作的这些时间部分的长度，通常是数百个时钟周期，定义了必须专门用于服务每个原子操作的最小时间量，并限制了吞吐量，或者执行原子操作的速率。

![image-20240726172602670](imgs/image-20240726172602670.png)

例如，假设一个内存系统有一个每个通道64位（8字节）的双数据速率DRAM接口，八个通道，1 GHz的时钟频率，以及典型的200周期的访问延迟。内存系统的峰值访问吞吐量是8（字节/传输）× 2（每个时钟每个通道的传输）× 1 G（每秒的时钟）× 8（通道）= 128 GB/s。假设每个访问的数据是4字节，系统有32 G数据元素每秒的峰值访问吞吐量。

然而，在对特定内存位置执行原子操作时，一个人可以达到的最高吞吐量是每400周期一个原子操作（200周期用于读取，200周期用于写入）。这转化为基于时间的吞吐量为1/400原子/时钟 × 1 G（每秒的时钟）= 2.5 M原子/秒。这比大多数用户对GPU内存系统的期望要低得多。此外，原子操作序列的长延迟可能会占据内核执行时间的主导地位，并可能大幅降低内核的执行速度。

在实践中，并非所有的原子操作都会在单一的内存位置执行。在我们的文本直方图示例中，直方图有七个区间。如果输入字符在字母表中均匀分布，原子操作将在histo元素中均匀分布。这将把吞吐量提高到7 × 2.5 M = 17.5 M原子操作每秒。实际上，由于字符在字母表中往往有偏差的分布，提升因子往往远低于直方图的区间数。例如，在图9.1中，我们看到示例短语中的字符严重偏向于m-p和q-t区间。更新这些区间的重争流量可能会将可实现的吞吐量降低到大约(28/10) × 2.5 M = 7 M。

提高原子操作吞吐量的一种方法是减少对高度争用位置的访问延迟。缓存存储器是减少内存访问延迟的主要工具。因此，现代GPU允许在所有流多处理器（SM）共享的最后一级缓存中执行原子操作。在原子操作期间，如果更新的变量在最后一级缓存中找到，它就会在缓存中更新。如果在最后一级缓存中找不到，它就会触发缓存未命中并被带入缓存，在那里更新。由于由原子操作更新的变量往往被许多线程高度访问，这些变量一旦从DRAM中被带入，就倾向于保留在缓存中。因为对最后一级缓存的访问时间是以数十个周期而不是数百个周期计算的，所以与早期的GPU相比，原子操作的吞吐量至少提高了十倍。这是大多数现代GPU支持在最后一级缓存中执行原子操作的重要原因。

## 9.4 Privatization

提高原子操作吞吐量的另一种方法是通过将流量从高度争用的位置转移来减轻争用。这可以通过一种称为私有化的技术来实现，该技术通常用于解决并行计算中的严重输出干扰问题。这个想法是将高度争用输出数据结构复制到私有副本中，以便每个线程子集可以更新其私有副本。好处是私有副本可以在更少的争用和通常更低的延迟下被访问。这些私有副本可以显著提高更新数据结构的吞吐量。缺点是私有副本需要在计算完成后合并到原始数据结构中。人们必须仔细平衡争用程度和合并成本之间的关系。因此，在大规模并行系统中，私有化通常是针对线程子集而不是单个线程进行的。

在我们的文本直方图示例中，我们可以创建多个私有直方图并指定每个直方图的线程子集进行更新。例如，我们可以创建两个私有副本，并让偶数索引的块更新其中一个，奇数索引的块更新另一个。另一个例子是，我们可以创建四个私有副本，并让索引为4n+i的块更新第i个私有版本，其中I=0，...，3。一个常见的方法是为每个线程块创建一个私有副本。这种方法有多个优点，我们将在后面看到。

![image-20240726174501200](imgs/image-20240726174501200.png)

图9.8展示了如何将私有化应用于图9.3中的文本直方图示例。在这个示例中，线程被组织成线程块，每个块由八个线程组成（实际上，线程块要大得多）。每个线程块收到一个它更新的私有直方图副本。如图9.8所示，不是在更新同一直方图bin的所有线程之间发生争用，而是只在同一块中的线程之间以及在最后合并私有副本时发生争用。

![image-20240726174819172](imgs/image-20240726174819172.png)

图9.9展示了一个简单的内核，它为每个块创建并关联一个私有直方图副本。在这个方案中，多达1024个线程将对直方图的一个副本进行工作。在这个内核中，私有直方图在全局内存中。这些私有副本可能会被缓存在L2缓存中，以减少延迟并提高吞吐量。

图9.9中的内核第一部分（第02-08行）与图9.6中的内核有一个关键区别。图9.9中的内核假设主机代码将为histo数组分配足够的设备内存来容纳所有私有直方图副本，这相当于gridDim.x×NUM_BINS×4字节。这反映在第06行，每个线程在执行原子添加时在索引上添加了一个偏移量blockIdx.x×NUM_BINS。这个偏移将位置转移到线程所属块的私有副本。在这种情况下，争用程度通过大约是所有SM上活动块数的因素来降低。争用减少的效果可以导致内核更新吞吐量的数倍提高。

在执行结束时，每个线程块将私有副本中的值提交到块0产生的版本中（第09-17行）。也就是说，我们将块0的私有副本提升为一个公共副本，将包含所有块的总结果。块中的线程首先等待彼此完成更新私有副本（第10行）。接下来，线程遍历私有直方图bin（第11行），每个线程负责提交一个或多个私有箱bin。使用循环来适应任意数量的bin。每个线程读取它负责的私有箱的bin（第13行），并检查bin是否非零（第13行）。如果是，线程通过将其原子地添加到块0的副本中来提交值（第14行）。请注意，添加需要原子执行，因为来自多个块的线程可以同时在同一个位置执行添加。因此，在内核执行结束时，最终的直方图将在histo数组的前NUM_BINS元素中。由于在这个阶段的内核执行期间，每个块只有一个线程会更新任何给定的histo数组元素，因此每个位置的争用程度非常适中。

按每个线程块创建直方图私有副本的一个好处是，线程可以使用__syncthreads()在提交之前相互等待。如果私有副本被多个块访问，我们将需要调用另一个内核来合并私有副本或使用其他复杂的技术。按每个线程块创建直方图私有副本的另一个好处是，如果直方图中的bin数足够小，直方图的私有副本可以在共享内存中声明。如果私有副本被多个块访问，使用共享内存将是不可能的，因为块不能看到彼此的共享内存。

回想一下，任何减少延迟都直接转化为同一内存位置原子操作的提高吞吐量。通过将数据放置在共享内存中，可以大幅减少访问内存的延迟。共享内存对每个SM都是私有的，并且有非常短的访问延迟（几个周期）。这种减少的延迟直接转化为原子操作吞吐量的增加。

![image-20240726175649789](imgs/image-20240726175649789.png)

图9.10展示了一个私有化直方图内核，它将私有副本存储在共享内存中，而不是全局内存中。与图9.9中的内核代码的关键区别在于，直方图的私有副本在histo_s数组中分配在共享内存中，并由块的线程并行初始化为0（第02-06行）。屏障同步（第07行）确保在任何线程开始更新之前，私有直方图的所有bin都已正确初始化。其余的代码与图9.9中的代码相同，只是第一个原子操作在共享内存数组histo_s的元素上执行（第13行），并且私有bin值稍后从那里读取（第19行）。

## 9.5 Coarsening

我们已经看到私有化在减少原子操作的竞争中是有效的，并且将私有化直方图存储在共享内存中可以减少每个原子操作的延迟。然而，私有化的开销是需要将私有副本提交到公共副本。这个提交操作每个线程块只执行一次。因此，我们使用的线程块越多，这个开销就越大。当线程块并行执行时，这个开销通常是值得支付的。然而，如果启动的线程块数量超过了硬件可以同时执行的数量，硬件调度将会使这些线程块序列化。在这种情况下，私有化开销就不必要地产生了。

我们可以通过线程粗化来减少私有化的开销。换句话说，我们可以通过减少块的数量并让每个线程处理多个输入元素来减少提交到公共副本的私有副本数量。在这一部分，我们看看将多个输入元素分配给一个线程的两种策略：连续分区和交错分区

![image-20240729141611215](imgs/image-20240729141611215.png)

![image-20240729141707500](imgs/image-20240729141707500.png)

图9.11显示了连续分区策略的一个例子。输入被划分为连续的段，每个段被分配给一个线程。

图9.12显示了使用连续分区策略应用粗化的直方图内核。与图9.10的区别在于09-10行。在图9.10中，输入元素索引i对应于全局线程索引，因此每个线程接收一个输入元素。在图9.11中，输入元素索引i是一个循环的索引，该循环从tid * CFACTOR迭代到(tid + 1) * CFACTOR，其中CFACTOR是粗化因子。因此，每个线程获取CFACTOR元素的连续段。循环边界中的min操作确保末端线程不会越界读取。

将数据划分为连续段在概念上是简单直观的。在CPU上，其中并行执行通常涉及少量线程，连续分区通常是表现最佳的策略，因为每个线程的顺序访问模式很好地利用了缓存行。由于每个CPU缓存通常只支持少量线程，不同线程对缓存使用的干扰很小。一旦数据被带入缓存行，可以期望它在后续访问中仍然保留在缓存中。

相比之下，在GPU上进行连续分区会导致次优的内存访问模式。正如我们在第5章“内存架构和数据局部性”中学到的，SM中同时活动的线程数量通常会引起如此多的缓存干扰，以至于人们不能期望数据在单个线程的所有顺序访问中都保留在缓存中。相反，我们需要确保warp中的线程访问连续的位置以启用内存合并。这一观察激发了交错分区。

![image-20240729142725205](imgs/image-20240729142725205.png)

图9.13显示了交错分区策略的一个例子。在第一次迭代中，八个线程访问字符0到7（“program”）。通过内存合并，所有元素只需一次DRAM访问即可获取。在第二次迭代中，四个线程在一个合并的内存访问中访问字符“ing mass”。很明显，这就是为什么这被称为交错分区：不同线程要处理的分区是相互交错的。显然，这是一个玩具示例，在现实中，将会有更多的线程。还有一些更微妙的性能考虑。例如，每个线程在每次迭代中应该处理四个字符（一个32位字）以充分利用缓存和SMs之间的互连带宽。

![image-20240729142914370](imgs/image-20240729142914370.png)

图9.14显示了使用交错分区策略应用粗化的直方图内核。与图9.10和9.12的区别再次在09-10行。在循环的第一次迭代中，每个线程使用其全局线程索引访问数据数组：线程0访问元素0，线程1访问元素1，线程2访问元素2，依此类推。因此，所有线程共同处理输入的前blockDim.x * gridDim.x个元素。在第二次迭代中，所有线程将blockDim.x * gridDim.x添加到它们的索引中，并共同处理下一部分的blockDim.x * gridDim.x个元素。

当线程的索引超过输入缓冲区的有效范围（其私有i变量的值大于或等于长度）时，线程已完成其分区的处理并将退出循环。由于缓冲区的大小可能不是线程总数的倍数，一些线程可能不参与处理最后一部分。因此，一些线程将比其他线程少执行一个循环迭代。

## 9.6 Aggregation 聚合

一些数据集在局部区域有大量相同数据值的集中。例如，在天空的图片中，可能会有大量像素值相同的区域。这样高浓度的相同值会导致并行直方图计算的竞争和吞吐量降低。

对于这样的数据集，一个简单而有效的优化是，如果每个线程都在更新直方图的同一个元素，那么每个线程就可以将连续的更新聚合成单个更新（Merrill, 2015）。这种聚合减少了对高度竞争的直方图元素的原子操作数量，从而提高了计算的有效吞吐量。

![image-20240729143909403](imgs/image-20240729143909403.png)

图9.15显示了一个聚合的文本直方图内核。与图9.14中的内核相比，关键变化如下：每个线程声明了一个额外的累加器变量（第09行），用于跟踪迄今为止聚合的更新数量，以及一个prevBinIdx变量（第10行），用于跟踪最后遇到的并正在聚合的直方图bin的索引。每个线程将累加器变量初始化为零，表示最初没有聚合任何更新，并将prevBinIdx设置为-1，以便没有字母输入会匹配它。

当发现字母数据时，线程将待更新的直方图元素的索引与正在聚合的索引进行比较（第16行）。如果索引相同，线程只需增加累加器（第17行），将聚合更新的连续性延长一个。如果索引不同，则对直方图元素的聚合更新序列已经结束。线程使用原子操作将累加器值添加到由prevBinIdx跟踪索引的直方图元素（第19-21行）。这有效地清空了之前聚合更新序列的总贡献。

通过这种方案，更新总是至少落后一个元素。在没有连续性的极端情况下，所有的更新将始终落后一个元素。这就是为什么在线程完成扫描所有输入元素并退出循环后，线程需要检查是否需要清空累加器值（第27行）。如果是这样，累加器值将被清空到正确的histo_s元素（第28行）。

一个重要的观察是，聚合内核需要更多的语句和变量。因此，如果竞争率很低，聚合内核可能比简单内核执行速度慢。然而，如果数据分布导致原子操作执行的竞争，聚合可能导致显著更高的速度。增加的if语句可能会表现出控制发散。然而，如果没有竞争，控制发散将很小，因为线程要么都在清空累加器值，要么都在连续性中。在一些线程处于连续状态，一些线程正在清空它们的累加器值的情况下，控制发散可能会因减少的竞争而得到补偿。

## 9.7 总结

计算直方图对于分析大型数据集非常重要。它也代表了一类重要的并行计算模式，其中每个线程的输出位置是数据依赖的，这使得应用所有者计算规则变得不可行。因此，它是引入读写修改竞争条件概念以及实际使用原子操作的自然载体，这些操作确保了对同一内存位置的并发读写修改操作的完整性。

不幸的是，正如我们在本章中解释的，原子操作的吞吐量比简单的内存读写操作要低得多，因为它们的吞吐量大约为两倍内存延迟的倒数。因此，在严重竞争的情况下，直方图计算的计算吞吐量可能会出奇地低。私有化作为一种重要的优化技术被引入，它系统地减少了竞争，并可以进一步启用共享内存，支持低延迟，从而实现高吞吐量。事实上，支持块内线程之间的快速原子操作是共享内存的重要用例。粗化也被应用于减少需要合并的私有副本数量，并比较了使用连续分区和交错分区的不同粗化策略。最后，对于导致严重竞争的数据集，聚合也可以显著提高执行速度。

# 10 Reduction

规约是从一组值中派生出一个单一的值。这个单一的值可以是所有元素的总和、最大值、最小值等。这个值也可以是各种类型：整数、单精度浮点数、双精度浮点数、半精度浮点数、字符等。所有这些类型的规约都具有相同的计算结构。
像直方图一样，规约是一个重要的计算模式，因为它从大量数据中生成摘要。并行规约是一个重要的并行模式，它要求并行线程彼此协调以获得正确的结果。这种协调必须小心进行，以避免性能瓶颈，这在并行计算系统中很常见。因此，并行规约是一个很好的工具，用来说明这些性能瓶颈，并介绍减轻它们的技术。

## 10.1 背景

数学上，如果一个二元运算符具有一个明确定义的恒等值，那么可以为一组项定义规约。例如，浮点加法运算符的恒等值是0.0；也就是说，任何浮点值v与值0.0相加的结果都是v本身。因此，可以基于加法运算符为一组浮点数定义规约，以产生集合中所有浮点数的总和。例如，对于集合{7.0, 2.1, 5.3, 9.0, 11.2}，求和规约将产生7.0+2.1+5.3+9.0+11.2 = 34.6。

规约可以通过顺序遍历数组的每个元素来执行。图10.1显示了C语言中顺序求和规约的代码。代码将结果变量sum初始化为恒等值0.0。然后使用for循环遍历包含值集合的输入数组。在第i次迭代期间，代码对sum的当前值和输入数组的第i个元素执行加法运算。以我们的例子来说，在第0次迭代后，sum变量包含0.0+7.0 = 7.0。在第1次迭代后，sum变量包含7.0+2.1 = 9.1。因此，在每次迭代后，输入数组的另一个值将被加到（累积到）sum变量中。在第5次迭代后，sum变量包含34.6，这是规约结果。

![image-20240730112805319](imgs/image-20240730112805319.png)

可以为许多其他运算符定义规约。可以为浮点乘法运算符定义乘积规约，其恒等值是1.0。一组浮点数的乘积规约是这些数的乘积。可以为最小比较运算符定义最小（min）规约，该运算符返回两个输入中的较小值。对于实数，最小运算符的恒等值是+∞。可以为最大（max）比较运算符定义最大（max）规约，该运算符返回两个输入中较大的值。对于实数，最大运算符的恒等值是-∞。

![image-20240730112905988](imgs/image-20240730112905988.png)

图10.2展示了一个运算符的一般形式的规约，该运算符被定义为一个接受两个输入并返回一个值的函数。在for循环的迭代过程中访问一个元素时，要采取的操作取决于正在执行的规约类型。例如，对于最大值规约，运算符函数在两个输入之间执行比较，并返回两者中较大的值。对于最小值规约，运算符函数比较两个输入的值，并返回较小的值。当for循环访问完所有元素时，顺序算法结束。对于一组N个元素，for循环迭代N次，并在循环结束时产生规约结果。

## 10.2 Reduction trees

![image-20240730113029274](imgs/image-20240730113029274.png)

图10.3展示了并行规约的基本概念，其中时间向下垂直推进，线程在每个时间步骤中并行执行的活动在水平方向上显示。
在第一轮（时间步骤）中，四个最大值运算并行执行在原始元素的四对上。这四个运算产生部分规约结果：原始元素四对中四个较大的值。在第二时间步骤中，并行执行两个最大值运算在部分规约结果的两对上，并产生两个更接近最终规约结果的部分结果。这两个部分结果分别是原始输入中前四个元素中的最大值和后四个元素中的最大值。在第三轮也是最后的时间步骤中，执行一个最大值运算以生成最终结果，原始输入中的最大值7。

请注意，当您从顺序规约算法转变为并行规约算法时，执行操作的顺序将会改变。例如，在图10.3顶部的输入中，像图10.2中那样的顺序最大值规约会首先将累积值（acc）中的恒等值（$-\infty$）与输入值3进行比较，并用胜者3更新acc。然后，它将比较acc的值（3）与输入值1，并用胜者3更新acc。这将被跟随比较acc的值（3）与输入值7，并用胜者7更新acc。然而，在图10.3的并行规约中，输入值7首先与输入值0进行比较，然后才与3和1的最大值进行比较。

正如我们所看到的，平行规约假定对输入值应用运算符的顺序并不重要。在最大值规约中，无论应用运算符到输入值的顺序是什么，结果都将是相同的。如果运算符是结合的，那么这个属性在数学上是有保证的。如果一个运算符Θ是结合的，那么(a Θ b) Θ c = a Θ (b Θ c)。例如，整数加法是结合的（(1+2)+3 = 1+(2+3) = 6），而整数减法不是结合的（(1-2)-2 ≠ 1-(2-3)）。也就是说，如果一个运算符是结合的，那么可以在涉及运算符的表达式中的任意位置插入括号，结果都是相同的。严格来说，由于可能以不同的方式引入括号而导致结果四舍五入，浮点加法不是结合的。然而，大多数应用都接受如果浮点运算结果彼此之间的差异在可容忍范围内，则视为相同。这样的定义允许开发人员和编译器编写者将浮点加法视为结合运算符。感兴趣的读者可以参考附录A进行详细讨论

从图10.2的顺序规约到图10.3的规约树的转换要求运算符必须是结合的。我们可以将规约看作是一系列操作的列表。图10.2和图10.3之间顺序的差异仅仅是在相同列表的不同位置插入括号。对于图10.2，括号是： ((((((3 max 1) max 7) max 0) max 4) max 1) max 6) max 3 而图10.3的括号是： ((3 max 1) max (7 max 0)) max ((4 max 1) max (6 max 3))

我们将在10.4节中应用一个优化，它不仅重新排列了应用运算符的顺序，还重新排列了操作数的顺序。为了重新排列操作数的顺序，这个优化进一步要求运算符必须是可交换的。如果一个运算符是可交换的，那么a Θ b = b Θ a。也就是说，在表达式中可以重新排列操作数的位置，结果相同。注意，最大值运算符也是可交换的，像min、sum和product等许多其他运算符也是如此。显然，并非所有运算符都是可交换的。例如，加法是可交换的（1+2 = 2+1），而整数减法不是（1-2 ≠ 2-1）。

图10.3中的并行规约模式被称为规约树，因为它看起来像一棵树，其叶子是原始输入元素，而根是最终结果。"规约树"这个术语不应与树形数据结构混淆，树形数据结构中的节点是通过指针显式链接或通过分配位置隐式链接的。在规约树中，边是概念性的，反映了从一个时间步骤执行的操作到下一个时间步骤执行的操作之间的信息流。

并行操作在产生最终结果所需的时间步骤数量上，与顺序代码相比有了显著的改进。在图10.3的例子中，顺序代码中的for循环迭代了八次，或花费了八个时间步骤，来访问所有输入元素并产生最终结果。另一方面，在图10.3中的并行操作，采用并行规约树方法只需要三个时间步骤：第一个时间步骤进行四次最大值操作，第二个时间步骤进行两次，第三个时间步骤进行一次。这在时间步骤数量上减少了5/8 = 62.5%，或者加速了8/3 = 2.67倍。当然，采用并行方法也有成本：必须有足够的硬件比较器，以便在同一时间步骤中执行多达四次的最大值操作。对于N个输入值，规约树在第一轮执行1/2 N次操作，在第二轮执行1/4 N次操作，以此类推。因此，执行的总操作数由几何级数1/2 N+1 + 1/4 N+1 / 8 N+... 1/N N = N - 1操作定义，这与顺序算法相似

在时间步骤方面，规约树需要$log_2(N)$步来完成N个输入值的规约过程。因此，它可以在仅仅十步内减少N = 1024个输入值，假设有足够的执行资源。在第一步，我们需要1/2 N = 512个执行资源！请注意，随着我们在时间步骤中的进展，所需的资源数量迅速减少。在最后的时间步骤，我们只需要一个执行资源。每个时间步骤的并行级别与所需的执行单元数量相同。有趣的是，计算所有时间步骤的平均并行级别。平均并行性是执行的操作总数除以时间步骤数，即$(N-1)/log_2(N)$。对于N = 1024，十个时间步骤的平均并行性是102.3，而峰值并行性是512（在第一步）。这种并行级别和资源消耗在时间步骤中的变化使得规约树成为并行计算系统中具有挑战性的并行模式。

![image-20240730145209795](imgs/image-20240730145209795.png)

图10.5显示了八个输入值的求和规约树示例。到目前为止，我们介绍的关于最大值规约树的时间步骤数和消耗的资源数量也适用于求和规约树。使用最多四个加法器，需要$log_2(8)=3$个时间步骤来完成规约。我们将使用这个例子来说明求和规约核的设计。

> 体育和竞赛中的并行规约 早在计算机出现之前，体育和竞赛中就已经使用了并行规约。图10.4展示了2010年南非世界杯四分之一决赛、半决赛和决赛的赛程。应该清楚的是，这只是一棵重新排列的规约树。世界杯的淘汰过程是一个最大值规约，其中最大值运算符返回“击败”另一支队伍的队伍。比赛的“规约”是在多轮中进行的。队伍被分成对。在第一轮，所有对同时进行比赛。第一轮的胜者进入第二轮，第二轮的胜者进入第三轮，以此类推。有八支队伍参加比赛，第一轮（图10.4中的四分之一决赛）将有四个胜者出现，第二轮（图10.4中的半决赛）将有两个胜者，第三轮（图10.4中的决赛）将有一个最终胜者（冠军）。每一轮都是规约过程的一个时间步骤。
>
> ![image-20240730145402549](imgs/image-20240730145402549.png)
>
> 即使有1024支队伍，也只需要10轮就能确定最终的胜者。关键是要有足够的资源，在第一轮同时举行512场比赛，在第二轮举行256场比赛，在第三轮举行128场比赛，以此类推。只要有足够的资源，即使有60000支队伍，我们也只需16轮就能确定最终的胜者。有趣的是，虽然规约树可以大大加快规约过程，但它们也消耗相当多的资源。在世界杯的例子中，一场比赛需要一个大型足球场、官员和工作人员，以及酒店和餐馆来容纳观众中的大量球迷。图10.4中的四场四分之一决赛在三个城市（纳尔逊·曼德拉湾/伊丽莎白港、开普敦和约翰内斯堡）进行，这三个城市共同提供了足够的资源来举办这四场比赛。请注意，约翰内斯堡的两场比赛是在两天不同的时间进行的。因此，两场比赛之间共享资源使得规约过程花费了更多的时间。我们将在计算规约树中看到类似的权衡

## 10.3 A simple reduction kernel

我们现在准备开发一个简单的内核来执行图10.5所示的并行求和归约树。由于归约树需要所有线程之间的协作，而这在整张网格中是不可能的，我们将首先实现一个内核，该内核在单个块内执行求和归约树。也就是说，对于一个包含N个元素的输入数组，我们将调用这个简单的内核，并启动一个包含2N个线程的单个块的网格。由于一个块最多可以有1024个线程，我们可以处理最多2048个输入元素。我们将在第10.8节中消除这个限制。在第一次时间步长中，所有的2N个线程将参与进来，每个线程将两个元素相加，产生N/2个部分和。在下一个时间步长中，一半的线程将退出，只有N/4线程将继续参与，以产生N/4个部分和。这个过程将持续进行，直到最后一个时间步长，只剩下一个线程，并产生总和。

![image-20240807112919055](imgs/image-20240807112919055.png)

![image-20240807112955520](imgs/image-20240807112955520.png)

图10.6显示了简单求和内核函数的代码，图10.7说明了由这段代码实现的归约树的执行。请注意，在图10.7中，时间从上到下推进。我们假设输入数组位于全局内存中，当调用内核函数时，数组的指针作为参数传递。每个线程被分配到数据位置2×threadIdx.x（第02行）。也就是说，线程被分配到输入数组的偶数位置：线程0到input[0]，线程1到input[2]，线程2到input[4]，以此类推，如图10.7的顶部行所示。每个线程将是它被分配位置的“所有者”，并且是唯一一个写入该位置的线程。内核的设计遵循“所有者计算”的方法，其中每个数据位置由一个唯一的线程拥有，并且只能由那个所有者线程更新。

图10.7的顶部行显示了线程分配到输入数组的位置，以及随后每一行显示了在每个时间步长中对输入数组位置的写入，也就是说，图10.6中的for循环的每次迭代（第03行）。内核在for循环的每次迭代中覆盖的位置在图10.7中被标记为输入数组的填充位置。例如，在第一次迭代结束时，具有偶数索引的位置被覆盖为输入数组中原始元素对的部分和（0-1, 2-3, 4-5等）。在第二次迭代结束时，索引为4的倍数的位置被覆盖为输入数组中四个相邻原始元素的部分和（0-3, 4-7等）。

在图10.6中，使用了步长变量来让线程获取适当的部分和，以便积累到它们拥有的位置。步长变量初始化为1（第03行）。步长变量的值在每次迭代中翻倍，因此步长变量的值将是1、2、4、8等，直到它大于blockIdx.x，即块中线程的总数。如图10.7所示，每次迭代中的每个活动线程使用步长变量将其拥有的位置的输入数组元素与步长距离的元素相加。例如，在第一次迭代中，线程0使用步长值1将input[1]加到它拥有的位置input[0]。这将input[0]更新为输入数组中input[0]和input[1]原始值的第一对部分和。在第二次迭代中，线程0使用步长值2将input[2]加到input[0]。此时，input[2]包含input[2]和input[3]原始值的和，而input[0]包含input[0]和input[1]原始值的和。因此，在第二次迭代后，input[0]包含输入数组前四个元素的原始值的和。在最后一次迭代后，input[0]包含输入数组所有原始元素的和，因此是求和归约的结果。这个值由线程0作为最终输出写入（第10行）。

我们现在转向图10.6中的if语句（第04行）。if语句的条件设置为在每次迭代中选择活动线程。如图10.7所示，在第n次迭代期间，线程索引（threadIdx.x）值是2n的倍数的线程将执行加法。条件是threadIdx.x % stride == 0，这测试线程索引值是否是步长变量值的倍数。回想一下，步长的值在迭代中是1、2、4、8等，或者对于第n次迭代是2n。因此，条件确实测试线程索引值是否是2n的倍数。

回想一下，所有线程执行相同的内核代码。线程索引值满足if条件的线程是执行加法语句（第05行）的活动线程。线程索引值不满足条件的线程是不执行加法语句的非活动线程。随着迭代的进行，活动线程越来越少。在最后一次迭代中，只有线程0保持活动，并产生求和归约结果。

\_\_syncthreads()语句（图10.6的第07行）在for循环中确保所有由迭代计算的部分和已经被写入输入数组的目标位置，然后才允许任何线程开始下一次迭代。这样，进入迭代的所有线程将能够正确使用上一次迭代产生的部分和。例如，在第一次迭代后，偶数元素将被替换为成对的部分和。__syncthreads()语句确保所有这些第一次迭代的部分和确实已经被写入输入数组的偶数位置，并准备好被第二次迭代中的活动线程使用。

## 10.4 Minimizing control divergence

图10.6中的内核代码实现了图10.7中的并行归约树，并产生了预期的求和归约结果。不幸的是，它在每次迭代中对活动线程和非活动线程的管理导致了高度的控制发散。例如，如图10.7所示，在第二次迭代期间，只有那些threadIdx.x值为偶数的线程将执行加法语句。正如我们在第4章《计算架构与调度》中解释的，控制发散可以显著降低执行资源利用效率，即用于生成有用结果的资源百分比。在这个例子中，一个warp中的所有32个线程消耗执行资源，但只有一半是活动的，浪费了一半的执行资源。由于发散导致的执行资源浪费随着时间的推移而增加。在第三次迭代中，一个warp中只有四分之一的线程是活动的，浪费了四分之三的执行资源。在第五次迭代中，一个warp中的32个线程中只有一个是活动的，浪费了32分之31的执行资源。

如果输入数组的大小大于32，整个warp在第五次迭代后将变得不活动。例如，对于256的输入大小，将启动128个线程或四个warp。所有四个warp将具有相同的发散模式，正如我们在前一段中对第1至第5次迭代所解释的。在第六次迭代期间，warp 1和warp 3将完全变得不活动，因此不表现出控制发散。另一方面，warp 0和warp 2将只有一个活动线程，表现出控制发散并浪费了32分之31的执行资源。在第七次迭代中，只有warp 0是活动的，表现出控制发散并浪费了32分之31的执行资源。

一般来说，对于大小为N的输入数组，执行资源利用效率可以通过活动线程总数与消耗的总执行资源之间的比率来计算。消耗的总执行资源与所有迭代中活动warp的总数成正比，因为每个活动warp，无论其线程有多少是活动的，都会消耗全部执行资源。这个数字可以按以下方式计算：

![image-20240807171035232](imgs/image-20240807171035232.png)

在这里，N/64 是启动的warp的总数，因为会启动 N/2个线程，并且每32个线程形成一个warp。N/64这个项乘以5，因为所有启动的warp在前五次迭代中都是活动的。第五次迭代之后，每连续迭代warp的数量减半。括号内的表达式给出了所有迭代中活动warp的总数。第二项反映了每个活动warp为所有32个线程消耗了全部执行资源，无论这些warp中的活动线程数量如何。对于256大小的输入数组，消耗的执行资源是 (4×5+2+1)×32≈736。

活动线程在所有迭代中提交的执行结果数量是：

![image-20240807172136046](imgs/image-20240807172136046.png)

括号内的项给出了所有N/64个warp在前五次迭代中的活动线程。从第六次迭代开始，每次迭代活动warp的数量减半，每个活动warp中只有一个活动线程。对于256大小的输入数组，提交结果的总数是 4×(32+16+8+4+2+1)+2+1=255。这个结果应该是直观的，因为将256个值归约所需的总操作数是255。

 将前两个结果结合起来，我们发现对于256大小的输入数组，执行资源利用效率是 255/736≈0.35。这个比率说明，平行执行资源没有充分发挥其在加速这一计算中的潜力。平均来说，只有大约35%的消耗资源有助于求和归约结果。也就是说，我们只使用了大约35%的硬件潜力来加速计算。

基于这个分析，我们可以看到跨warp和时间的广泛控制发散。正如读者可能已经想到的，可能有更好的方法来分配线程到输入数组的位置，以减少控制发散并提高资源利用效率。图10.7中所示的分配问题在于，部分和位置彼此之间的距离越来越远，因此随着时间的推移，拥有这些位置的活动线程也越来越远。活动线程之间距离的增加有助于控制发散水平的提高。

确实有更好的分配策略，可以显著减少控制发散。这个想法是我们应该安排线程和它们拥有的位置，以便它们可以随着时间的推移保持彼此接近。也就是说，我们希望步长值随着时间的推移而减小，而不是增加。修订后的分配策略在图10.8中针对16个元素的输入数组进行了展示。在这里，我们将线程分配到位置的前半部分。在第一次迭代中，每个线程跨越输入数组的一半，并将其添加到其拥有的位置。在我们的示例中，线程0将input[8]添加到其拥有的位置input[0]，线程1将input[9]添加到其拥有的位置input[1]，以此类推。在每次后续迭代中，一半的活动线程退出，所有剩余的活动线程添加一个输入元素，其位置是活动线程数量远离其拥有位置的。在我们的示例中，在第三次迭代中，有两个剩余的活动线程：线程0将input[2]添加到其拥有的位置input[0]，线程1将input[3]添加到其拥有的位置input[1]。注意，如果我们比较图10.8与图10.7的操作和操作数顺序，实际上是在列表中重新排序操作数，而不仅仅是以不同的方式插入括号。为了在这样的重新排序下始终保持相同的结果，操作必须是可交换的，并且是可结合的。

![image-20240807172809180](imgs/image-20240807172809180.png)

图10.9展示了一个内核，它与图10.6中的简单内核有一些微妙但关键的变化。拥有位置变量i被设置为threadIdx.x而不是 2×threadIdx.x（第02行）。因此，现在所有线程的拥有位置彼此相邻，如图10.8所示。步长值被初始化为blockDim.x，并在达到1之前减半（第03行）。在每次迭代中，只有索引小于步长值的线程保持活动（第04行）。因此，所有活动线程都具有连续的线程索引，如图10.8所示。与第一轮添加邻居元素不同，它添加的是彼此相隔半个区段的元素，并且区段大小始终是剩余活动线程数量的两倍。在第一轮添加的所有对在彼此之间相隔blockDim.x。在第一次迭代后，所有的成对和都存储在输入数组的前半部分，如图10.8所示。循环在进入下一次迭代之前将步长除以2。因此，在第二次迭代中，步长变量值是blockDim.x值的一半。也就是说，在第二次迭代期间，剩余的活动线程添加的是彼此相隔四分之一区段的元素。

![image-20240807172929906](imgs/image-20240807172929906.png)

图10.9中的内核仍然有一个if语句（第04行）在循环中。每次迭代中执行加法操作（第06行）的线程数量与图10.6中的相同。那么，两个内核之间的控制发散为什么会有差异呢？答案在于执行加法操作的线程相对于不执行的线程的位置。让我们考虑一个256元素输入数组的例子。在第一次迭代中，所有线程都是活动的，所以没有控制发散。在第二次迭代中，线程0到63执行加法语句（活动），而线程64到127不执行（不活动）。在第二次迭代期间，成对的和存储在元素0到63中。由于warp由具有连续threadIdx.x值的32个线程组成，因此warp 0到warp 1中的所有线程执行加法语句，而warp 2到warp 3中的所有线程变得不活动。由于每个warp中的所有线程采取相同的执行路径，所以没有控制发散！

然而，图10.9中的内核并没有完全消除由if语句引起的发散。读者应该验证，对于256元素的例子，从第四次迭代开始，执行加法操作的线程数量将低于32。也就是说，最后的五次迭代将只有16、8、4、2和1个线程执行加法。这意味着内核执行仍然在这几次迭代中有发散。然而，具有发散的循环迭代次数从十次减少到了五次。我们可以按以下方式计算消耗的总执行资源：

![image-20240807173203318](imgs/image-20240807173203318.png)

括号内的部分反映了这样一个事实：在每次后续迭代中，一半的warp变得完全不活动，不再消耗执行资源。这个序列一直持续到只剩下一个完整的活动线程warp。最后一个项（5×1）反映了这样一个事实：在最后五次迭代中，只有一个活动的warp，它的所有32个线程都在消耗执行资源，尽管只有一部分线程是活动的。因此，括号内的总和给出了所有迭代中warp执行的总数，乘以32，就得到了消耗的总执行资源量。

对于我们的256元素示例，消耗的执行资源是 (4+2+1+5×1)×32=384，这是图10.6中的内核消耗的736资源的将近一半。由于图10.7到图10.8中每次迭代的活动线程数量没有变化，图10.9中新内核的效率是 255/384≈66%，这几乎是图10.6中内核效率的两倍。还要注意，由于warp是在有限执行资源的流式多处理器上轮流执行的，因此随着资源消耗的减少，总的执行时间也会有所改善。

图10.6和图10.9中的内核之间的差异很小，但可能会对性能产生显著影响。这需要对设备上单指令多数据硬件上线程执行有清晰理解的人，才能有信心进行这样的调整。

## 10.5 Minimizing memory divergence

图10.6中的简单内核有另一个性能问题：内存发散。正如我们在第5章《内存架构和数据局部性》中解释的，重要的是在每个warp内实现内存合并。也就是说，warp中的相邻线程在访问全局内存时应该访问相邻的位置。不幸的是，在图10.7中，相邻线程没有访问相邻的位置。

在每次迭代中，每个线程执行两次全局内存读取和一次全局内存写入。第一次读取来自其拥有的位置，第二次读取来自距离其拥有位置步长距离的位置，写入则是到其拥有的位置。由于相邻线程拥有的位置不是相邻位置，相邻线程所做的访问将不会完全合并。在每次迭代中，一个warp集体访问的内存位置彼此之间有步长距离。

例如，如图10.7所示，当一个warp中的所有线程在第一次迭代中执行它们的第一个读取时，这些位置彼此相隔两个元素。结果，触发了两次全局内存请求，返回的数据一半将不被线程使用。第二次读取和写入也发生了相同的行为。在第二次迭代中，每隔一个线程就退出，warp集体访问的位置彼此相隔四个元素。再次执行两次全局内存请求，返回的数据只有四分之一被线程使用。这将一直持续到每个warp中只剩下一个活动线程。只有当warp中有一个活动线程时，warp才会执行一次全局内存请求。因此，全局内存请求的总数如下：

![image-20240807173901489](imgs/image-20240807173901489.png)

第一项$$N/64 \times 5 \times 2$$对应于前五次迭代，在这些迭代中，所有\(N/64\)个warp（线程组）都有两个或更多的活跃线程，因此每个warp执行两次全局内存请求。剩余的项计算了最后的迭代，在这些迭代中，每个warp只有一个活跃线程并执行一次全局内存请求，并且在每次后续迭代中，有一半的warp退出。乘以3是因为每个活跃线程在每次迭代中进行两次读取和一次写入。在256元素的示例中，内核执行的全局内存请求总数是$$(4 \times 5 \times 2 + 4 + 2 + 1) \times 3 \times 5 = 141$$。

对于图10.9中的内核，每个warp中的相邻线程总是访问全局内存中的相邻位置，因此访问总是被合并的。结果，每个warp在任何读取或写入时只触发一次全局内存请求。随着迭代的进行，整个warp退出，因此这些不活跃的warp中的任何线程都不会执行全局内存访问。每次迭代中都有一半的warp退出，直到最后五次迭代只剩下一个warp。因此，内核执行的全局内存请求总数如下：

![image-20240807174237061](imgs/image-20240807174237061.png)

在256元素的示例中，执行的全局内存请求总数是$$(4+2+1+5) \times 3 = 36$$。改进后的内核导致全局内存请求数量减少了$$141/36 \approx 3.9$$倍。由于DRAM带宽是一种有限的资源，图10.6中的简单内核的执行时间可能会显著更长。

在2048元素的示例中，图10.6中的内核执行的全局内存请求总数是$$(32 \times 5 \times 2 + 32 + 16 + 8 + 4 + 2 + 1) \times 3 = 1149$$，而图10.9中的内核执行的全局内存请求总数是$$(32 + 16 + 8 + 4 + 2 + 1 + 5) \times 3 = 204$$。这个比例是5.6，甚至比256元素的示例还要高。这是因为图10.6中的内核执行模式效率低下，在执行的最初五次迭代中有更多的活跃warp，并且每个活跃warp触发的全局内存请求数量是图10.9中收敛内核的两倍。

总之，收敛内核在使用执行资源和DRAM带宽方面提供了更高的效率。这种优势来自于减少的控制发散和改进的内存合并。

## 10.6 Minimizing global memory accesses

图10.9中的收敛核可以通过使用共享内存进一步改进。注意，在每次迭代中，线程将其部分求和结果值写入全局内存，并且这些值在下一次迭代中被相同的线程和其他线程重新读取。由于共享内存的延迟远小于全局内存，且带宽更高，我们可以通过将部分求和结果保留在共享内存中来进一步提高执行速度。这一想法在图10.10中进行了说明。

![image-20240809162845741](imgs/image-20240809162845741.png)

![image-20240809162927014](imgs/image-20240809162927014.png)

使用共享内存的策略是在图10.11所示的核中实现的。这个想法是使用每个线程加载并添加两个原始元素，然后将部分求和写入共享内存（第04行）。由于第一次迭代已经在循环之外访问全局内存位置时完成，因此for循环从blockDim.x/2（第04行）而不是blockDim.x开始。将__syncthreads()移动到循环的开头，以确保我们在共享内存访问和循环的第一次迭代之间进行同步。线程通过读取和写入共享内存继续进行剩余的迭代（第08行）。最后，在核的末尾，线程0将求和写入输出，保持与之前核相同的行为（第11-13行）。

使用图10.11中的核，全局内存访问次数减少到输入数组原始内容的初始加载和对input[0]的最终写入。因此，对于一个N元素的归约，全局内存访问次数仅为N+1。还要注意，在图10.11（第04行）中的两个全局内存读取是合并的。因此，通过合并，将只有(N/32)+1个全局内存请求。对于256元素的例子，触发的全局内存请求的总数将从图10.9中的核的36个减少到图10.10中的共享内存核的8+1个，即9个，这是一个改进。使用共享内存的另一个好处是，除了减少全局内存访问次数外，输入数组不会被修改。如果程序的另一部分需要数组的原始值进行其他计算，这个属性是有用的.

## 10.7 Hierarchical reduction for arbitrary input length

我们到目前为止学习的所有内核都假设它们将使用一个线程块来启动。这种假设的主要原因是使用\_\_syncthreads()作为所有活动线程之间的屏障同步。回想一下，__syncthreads()只能在同一个块中的线程之间使用。这限制了当前硬件上的并行级别为1024个线程。对于包含数百万甚至数十亿元素的大型输入数组，我们可以通过启动更多线程来进一步加速归约过程。由于我们没有一种好的方法来在不同块中的线程之间执行屏障同步，我们将需要允许不同块中的线程独立执行。

![image-20240809163556194](imgs/image-20240809163556194.png)

![image-20240809163612440](imgs/image-20240809163612440.png)

图10.12展示了使用原子操作进行分层、分段的多块归约的概念，图10.13显示了相应的内核实现。这个想法是将输入数组划分为段，使得每个段的大小适合一个块。然后所有块独立执行一个归约树，并通过原子加操作将它们的结果累积到最终输出。

分区是根据线程的块索引（第03行）给段idx分配不同的值来完成的。每个段的大小是2倍的blockDim.x。也就是说，每个块处理2倍的blockDim.x个元素。因此，当我们将每个段的大小乘以块的blockIdx.x时，我们就得到了该块要处理的段的起始位置。例如，如果我们在一个块中有1024个线程，那么段的大小将是2乘以1024等于2048。段的起始位置将是块0为0，块1为2048（2048乘以1），块2为4096（2048乘以2），以此类推。

一旦我们知道了每个块的起始位置，所有块中的线程就可以简单地在分配给它们的段上工作，就好像它是整个输入数据一样。在块内，我们通过将threadIdx.x加到块的段起始位置来为每个线程分配拥有的位置（第04行）。局部变量i保存线程在全局输入数组中拥有的位置，而t保存线程在共享输入数组input_s中拥有的位置。第06行被调整为使用i而不是t来访问全局输入数组。图10.11中的for循环没有变化地被使用。这是因为每个块都有自己的私有input_s在共享内存中，所以它可以被访问，就好像段是整个输入一样。

一旦完成归约树的for循环，段的部分和就在input_s[0]中。图10.13中的第16行的if语句选择线程0将input_s[0]中的值贡献到输出，如图10.12的底部所示。这是通过图10.13的第14行所示的原子加法来完成的。

一旦网格的所有块完成执行，内核将返回，总和就在由output指向的内存位置中。

## 10.8 Thread coarsening for reduced overhead

我们迄今为止所使用的归约内核都试图通过尽可能多地使用线程来最大化并行性。也就是说，对于N个元素的归约，会启动N/2个线程。如果线程块的大小是1024个线程，那么得到的线程块数量是N/2048。然而，在执行资源有限的处理器上，硬件可能只有足够的资源来并行执行部分线程块。在这种情况下，硬件将串行化剩余的线程块，每当一个旧的线程块完成时，就执行一个新的线程块。

为了并行化归约，我们实际上付出了巨大的代价来将工作分配到多个线程块上。正如我们在早期部分看到的，由于更多的warp变得空闲，以及最终warp经历更多的控制发散，硬件的低效使用随着归约树的每个连续阶段而增加。硬件低效使用的阶段发生在我们启动的每个线程块中。如果线程块实际上要并行运行，这是不可避免的代价。然而，如果硬件要串行化这些线程块，我们最好自己以更有效的方式进行串行化。正如我们在第6章，性能考虑中讨论的，线程粒度粗化，或者简称为线程粗化，是一种优化类别，它将一些工作串行化到较少的线程中以减少并行化开销。我们首先展示一个应用线程粗化的并行归约实现，通过为每个线程块分配更多的元素。然后我们进一步阐述这种实现如何减少硬件低效使用。

![image-20240820112322123](imgs/image-20240820112322123.png)

图10.14展示了如何将线程粗化应用于图10.10中的例子。在图10.10中，每个线程块接收16个元素，即每个线程两个元素。每个线程独立地添加它负责的两个元素；然后线程协作执行归约树。在图10.14中，我们将线程块粗化了2倍。因此，每个线程块接收的元素数量是原来的两倍，即32个元素，也就是每个线程四个元素。在这种情况下，每个线程在线程协作执行归约树之前独立地添加四个元素。图10.14中前两行箭头展示了添加四个元素的三个步骤。注意，在这三个步骤中，所有线程都是活跃的。此外，由于线程独立地添加它们负责的四个元素，它们不需要同步，并且在添加完所有四个元素之前，也不需要将它们的部分和存储到共享内存中。执行归约树的剩余步骤与图10.10中的步骤相同。

![image-20240820112647586](imgs/image-20240820112647586.png)

图10.15显示了实现多块分段内核的线程粗化归约的内核代码。与图10.13相比，内核有两个主要的不同之处。第一个不同之处是，在确定块的段的开始时，我们乘以COARSE_FACTOR以反映块的段的大小是原来的COARSE_FACTOR倍（第03行）。第二个不同之处是，在添加线程负责的元素时，不是只添加两个元素（图10.13的第06行），我们使用一个粗化循环来迭代元素，并根据COARSE_FACTOR添加它们（图10.15的第06-09行）。请注意，在粗化循环的整个过程中，所有线程都是活跃的，部分和累积到局部变量sum中，并且在循环中没有调用__syncthreads()，因为线程独立操作。

![image-20240820112855832](imgs/image-20240820112855832.png)

图10.16比较了两个原始线程块（没有粗化）由硬件串行化执行的情况（图10.16A所示）与一个粗化线程块执行两个线程块的工作（图10.16B所示）。在图10.16A中，第一个线程块执行一个步骤，其中每个线程添加它负责的两个元素。在此步骤中，所有线程都处于活跃状态，因此硬件得到了充分利用。剩下的三个步骤执行归约树，其中每一步都有一半的线程退出，导致硬件的低效使用。此外，每一步都需要进行屏障同步以及访问共享内存。当第一个线程块完成后，硬件随后调度第二个线程块，它遵循相同的步骤，但作用在数据的不同部分。总体而言，这两个块共同需要总共八个步骤，其中两个步骤充分利用了硬件，而六个步骤则低效使用了硬件，并需要屏障同步和共享内存访问。

相比之下，在图10.16B中，相同的数据量仅由一个粗化了2倍的单一线程块处理。这个线程块最初进行三个步骤，其中每个线程添加它负责的四个元素。在这全部三个步骤中，所有线程都处于活跃状态，因此硬件得到了充分利用，并且没有进行屏障同步或访问共享内存。剩下的三个步骤执行归约树，其中每一步都有一半的线程退出，导致硬件的低效使用，并且需要屏障同步和访问共享内存。总体而言，只需要六个步骤（而不是八个），其中三个步骤（而不是两个）充分利用了硬件，而三个步骤（而不是六个）低效使用了硬件，并需要屏障同步和共享内存访问。因此，线程粗化有效地减少了由于硬件低效使用、同步和访问共享内存而产生的开销。

从理论上讲，我们可以将粗化因子增加到两倍以上。然而，我们必须记住，当我们粗化线程时，将减少并行完成的工作量。因此，增加粗化因子将减少硬件正在利用的数据并行性的数量。如果我们增加太多的粗化因子，以至于我们启动的线程块少于硬件能够执行的数量，我们将无法充分利用并行硬件执行资源。最佳的粗化因子确保有足够的线程块来充分利用硬件，这通常取决于输入的总大小以及特定设备的特性。

## 10.9 Summary

并行归约模式非常重要，因为它在许多数据处理应用中起着关键作用。尽管顺序代码很简单，但应该清楚地向读者表明，为了实现大输入的高性能，需要使用几种技术，如减少发散的线程索引分配、使用共享内存减少全局内存访问、分段归约与原子操作，以及线程粗化。归约计算也是前缀和模式的重要基础，前缀和模式是并行化许多应用程序的重要算法组成部分，将是第11章，前缀和（扫描）的主题。

# 11  前缀和（扫描）--并行算法中工作效率的介绍

我们的下一个并行模式是前缀和，它通常也被称为扫描。并行扫描经常用于并行化看似顺序的操作，例如资源分配、工作分配和多项式求值。一般来说，如果一个计算自然地被描述为数学递归，其中序列中的每个项都是根据前一项来定义的，那么它很可能可以作为并行扫描操作来并行化。并行扫描在大规模并行计算中起着关键作用，原因很简单：应用程序中的任何顺序部分都可能严重限制应用程序的整体性能。许多这样的顺序部分可以转换成并行计算，并行扫描。因此，前缀和通常用作执行基数排序、快速排序、字符串比较、多项式求值、解决递归、树操作和流压缩的并行算法中的原始操作。基数排序的例子将在第13章，排序中介绍。

并行扫描是一个重要的并行模式的另一个原因是，它是一些并行算法执行的工作可能比顺序算法执行的工作具有更高复杂度的典型例子，导致需要在算法复杂度和并行化之间仔细权衡。正如我们将展示的，算法复杂度的轻微增加可能会使并行扫描在大型数据集上比顺序扫描运行得更慢。在“大数据”时代，这种考虑变得更加重要，其中庞大的数据集挑战了具有高计算复杂度的传统算法。

## 11.1 背景

![image-20240820114626678](imgs/image-20240820114626678.png)

例如，如果“![image-20240820114716082](imgs/image-20240820114716082.png)”表示加法，对输入数组 [3 1 7 0 4 1 6 3] 执行包容性扫描操作将返回 [3, 4, 11, 11, 15, 16, 22, 25]。“包容性”扫描这个名字来源于每个输出元素都包含了相应输入元素的影响。

我们可以用一个为一群人切香肠的例子来说明包容性扫描操作的应用。假设我们有一根40英寸的香肠要分给八个人。每个人根据英寸订购了不同数量的香肠：3、1、7、0、4、1、6和3英寸。也就是说，第0个人想要3英寸的香肠，第1个人想要1英寸，以此类推。我们可以顺序地或并行地切香肠。顺序的方式非常简单。我们首先为第0个人切下3英寸的部分。现在香肠剩下37英寸长。然后我们为第1个人切下1英寸的部分。香肠变成了36英寸长。我们可以继续切更多的部分，直到我们为第7个人服务了3英寸的部分。到那时，我们已经服务了总共25英寸的香肠，还剩下15英寸。

包容性扫描操作允许我们根据每个人订购的数量计算所有切割点的位置。也就是说，给定一个加法操作和一个订单输入数组 [3 1 7 0 4 1 6 3]，包容性扫描操作返回 [3 4 11 11 15 16 22 25]。返回数组中的数字是切割位置。有了这些信息，我们可以同时进行所有八次切割，生成每个人订购的部分。第一个切割点位于3英寸的位置，所以第一部分将是3英寸长，正如第0个人订购的那样。第二个切割点位于4英寸的位置；因此，第二部分将是1英寸长，正如第1个人订购的那样。最后一个切割点位于25英寸的位置，这将产生一个3英寸长的部分，因为前一个切割点位于22英寸的位置。这给了第7个人她订购的东西。请注意，由于所有切割点都是从扫描操作中知道的，所有切割可以并行完成或以任何任意顺序完成。

总结来说，直观地理解包容性扫描的方式是，操作接受一群人的请求，并确定所有切割点，允许一次性服务所有订单。订单可以是香肠、面包、露营地空间，或计算机中的连续内存块。只要我们能快速计算出所有切割点，所有订单都可以并行服务。

![image-20240820114940915](imgs/image-20240820114940915.png)

独占性扫描操作中，每个输出元素不包含相应输入元素的影响。第一个输出元素是运算符 ![image-20240820114716082](imgs/image-20240820114716082.png) 的单位元 i，而最后一个输出元素仅反映了直到 xn-2 的贡献。二元运算符的单位元定义为一个值，当用作输入操作数时，使操作生成的输出值与另一个输入操作数的值相同。在加法运算符的情况下，单位元是 0，因为任何数与零相加都会得到它自己。

 独占性扫描的应用与包容性扫描非常相似。独占性扫描提供的是稍微不同的信息。在香肠例子中，独占性扫描将返回 [0 3 4 11 11 15 16 22]，这些是切割部分的起始点。例如，第0个人的段落从0英寸点开始。另一个例子，第7个人的段落从22英寸点开始。起始点信息在内存分配等应用中很重要，其中分配的内存通过指向其起始点的指针返回给请求者。

注意，独占性扫描输出和包容性扫描输出之间的转换很容易。只需移动所有元素并填充一个元素即可。当从包容性转换为独占性时，只需将所有元素右移，并将第0个元素填充为单位值。当从独占性转换为包容性时，需要将所有元素左移，并将最后一个元素填充为最后一个输入元素 " 与最后一个输入元素。这只是一个方便的问题，我们可以根据是关心切割点还是部分的起始点，直接生成包容性或独占性扫描。因此，我们将只针对包容性扫描呈现并行算法和实现。

在我们呈现并行扫描算法及其实现之前，我们想展示一个顺序包容性扫描算法及其实现。我们将假设涉及的运算符是加法。图11.1中的代码假设输入元素在 x 数组中，输出元素将写入 y 数组。

![image-20240820165553212](imgs/image-20240820165553212.png)

代码将输出元素 y[0] 初始化为输入元素 x[0] 的值（第02行）。在循环的每次迭代中（第03-05行），循环体将一个更多的输入元素加到前一个输出元素（存储所有前一个输入元素的累积）以生成一个更多的输出元素。

应该清楚的是，图11.1中顺序实现的包容性扫描所做的工作与输入元素的数量成线性比例；也就是说，顺序算法的计算复杂度是 O(N)。

在11.2至11.5节中，我们将呈现执行并行分段扫描的替代算法，其中每个线程块将并行对输入数组中的元素段，即部分，执行扫描。然后，我们将在11.6和11.7节中呈现将分段扫描结果组合成整个输入数组的扫描输出的方法。

## 11.2 Parallel scan with the Kogge-Stone algorithm

我们从一个简单的并行包容性扫描算法开始，为每个输出元素执行一个归约操作。人们可能会倾向于使用每个线程来执行图10.2所示的顺序归约，以获得一个输出元素。毕竟，这允许所有输出元素的计算并行执行。不幸的是，这种方法不太可能比图11.1中的顺序扫描代码提高执行时间。这是因为计算 $y_{n-1}$ 需要 n 步，与顺序扫描代码执行的步数相同，而且归约中的每一步（迭代）涉及的工作量与顺序扫描的每一次迭代相同。由于并行程序的完成时间受最长时间线程的限制，这种方法不太可能比顺序扫描快。实际上，如果计算资源有限，这种简单并行扫描算法的执行时间可能比顺序算法长得多。计算成本，或者说执行的总操作数，对于提出的方法来说，很遗憾会更高。由于输出元素 i 的归约步骤数将是 i，所有线程执行的总步骤数将是<img src="imgs/image-20240820165833526.png" alt="image-20240820165833526" style="zoom: 50%;" />

也就是说，所提出的方法具有 O(N^2) 的计算复杂度，这比顺序扫描的复杂度 O(N) 高，同时没有提供加速。更高的计算复杂度意味着需要提供更多的执行资源。这显然是一个糟糕的主意。

更好的方法是调整第10章中的并行归约树，“归约和最小化发散”，用相关输入元素的归约树来计算每个输出元素。为每个输出元素设计归约树有多种方法。由于元素 i 的归约树涉及 i 次加法运算，除非我们找到一种在不同输出元素的归约树之间共享部分和的方法，否则这种方法仍会增加计算复杂度至 O(N^2)。我们提出了一种基于 Kogge-Stone 算法的共享方法，该算法最初是在20世纪70年代为设计快速加法电路而发明的（Kogge & Stone, 1973）。这个算法至今仍被用于高速计算机算术硬件的设计中。

![image-20240829155655387](imgs/image-20240829155655387.png)

图11.2所示的算法是一种原地扫描算法，它作用于最初包含输入元素的数组XY。它迭代地将数组内容发展为输出元素。在算法开始之前，我们假设XY[i]包含输入元素xi。经过k次迭代后，XY[i]将包含位置及之前多达$2^k$个输入元素的和。例如，经过一次迭代后，XY[i]将包含$x_{i-1}+x_i$；在第二次迭代结束时，XY[i]将包含$x_{i-3}+x_{i-2}+x_{i-1}+x_i$，以此类推。

图11.2展示了一个包含16个元素的输入示例的算法。每一条垂直线代表XY数组的一个元素，其中XY[0]位于最左边的位置。垂直方向表示迭代的进展，从图的顶部开始。根据定义，对于包含扫描（inclusive scan），y0是x0，所以XY[0]包含其最终答案。在第一次迭代中，除了XY[0]之外，每个位置都会接收到它当前的内容和它左侧邻居的内容之和。这在第一层加法运算符下方的标签框中得到了说明。因此，XY[i]包含xi21+xi。这在图11.2的第一行加法运算符下方的标签框中得到了反映。例如，经过第一次迭代后，XY[3]包含x2+x3，显示为$\sum{x_2...x_3}$。请注意，经过第一次迭代后，XY[1]等于x0+x1，这是该位置的最终答案。因此，在后续迭代中，XY[1]不应该再发生变化。

在第二次迭代中，除了XY[0]和XY[1]之外，每个位置都会接收到它当前的内容和距离两个元素的位置的内容之和。这在下一行加法运算符下方的标签框中得到了说明。因此，XY[i]变为$x_{i-3}+x_{i-2}+x_{i-1}+x_i$。例如，经过第二次迭代后，XY[3]变为x0+x1+x2+x3，显示为$\sum x0...x3$。请注意，经过第二次迭代后，XY[2]和XY[3]已经达到了它们的最终答案，并且在接下来的迭代中不需要再进行更改。读者被鼓励继续完成剩余的迭代

![image-20240829160611360](imgs/image-20240829160611360.png)

图11.3展示了图11.2中算法的并行实现。我们实现了一个内核，它对输入的不同段（部分）执行局部扫描，每个段都足够小，可以由单个块处理。稍后，我们将进行最终调整，以整合这些分段扫描结果，用于大型输入数组。一个段的大小被定义为编译时常量SECTION_SIZE。我们假设内核函数将使用SECTION_SIZE作为块大小被调用，因此将有相同数量的线程和段元素。我们分配每个线程来发展一个XY元素的内容。

图11.3所示的实现假设输入值最初在全局内存数组X中，其地址作为参数传递到内核中（第01行）。我们将让块中的所有线程协作将X数组元素加载到共享内存数组XY中（第02行）。这是通过让每个线程计算其全局数据索引i=blockIdx.x * blockDim.x + threadIdx.x（第03行）来完成的，用于它负责的输出向量元素位置。每个线程在内核开始时将该位置的输入元素加载到共享内存中（第04到08行）。在内核结束时，每个线程将其结果写入分配的输出数组Y中（第18到20行）。

我们现在专注于图11.3中每个XY元素的迭代计算的实现，作为一个for循环（第09到17行）。循环遍历分配给线程的XY数组位置的归约树。当步长值大于线程的threadIdx.x值时，这意味着线程分配的XY位置已经累积了所有所需的输入值，线程不再需要活跃（第12和15行）。请注意，我们使用屏障同步（第10行）以确保所有线程在开始下一次迭代之前完成上一次迭代。这与第10章“归约和最小化发散”中归约讨论中__syncthreads()的用法相同。

然而，与归约相比，在更新XY元素（第12到16行）时有一个非常重要的不同。请注意，每个活跃线程首先将其位置的部分和存储到一个临时变量（在寄存器中）。在所有线程完成了第二次屏障同步（第14行）之后，它们都将部分和值存储到它们的XY位置（第16行）。额外的临时变量和__syncthreads()的需要与这些更新中的写后读数据依赖性危险有关。每个活跃线程添加它自己位置的XY值（XY[threadIdx.x]）和另一个线程位置的XY值（XY[threadIdx.x-stride]）。如果线程i在另一个线程i+stride有机会读取该位置的旧值之前写入其输出位置，新值可能会破坏另一个线程执行的加法。这种破坏可能会也可能不会发生，取决于涉及线程的执行时机，这被称为竞态条件。请注意，这种竞态条件与我们在第9章“并行直方图”中看到的竞态条件不同，那里的竞态条件是一个读写修改竞态条件，可以用原子操作解决。对于我们在这里看到的写后读竞态条件，需要不同的解决方案。

竞态条件可以在图11.2中很容易地观察到。让我们检查线程4（x4）和线程6（x6）在第二次迭代期间的活动，这在从上往下第二行的加法操作中表示。请注意，线程6需要将XY[4]的旧值（x3+x4）加到XY[6]的旧值（x5+x6）上，以生成XY[6]的新值（x3+x4+x5+x6）。然而，如果线程4过早地将其迭代的加法结果（x1+x2+x3+x4）存储到XY[4]中，线程6最终可能会使用新值作为其输入，并将（x1+x2+x3+x4+x5+x6）存储到XY[6]中。由于线程6在第三次迭代中将再次将x1+x2加到XY[6]中，XY[6]中的最终答案将变成（2x1+2x2+x3+x4+x5+x6），这显然是不正确的。另一方面，如果线程6在第二次迭代期间在线程4覆盖它之前碰巧读取了XY[4]中的旧值，结果将是正确的。也就是说，代码的执行结果可能是正确的，也可能不是，这取决于线程执行的时机，执行结果可能因运行而异。这种不可重现性可能使调试成为一场噩梦。

过在第13行使用的临时变量和第14行的\_\_syncthreads()屏障，克服了竞态条件。在第13行，所有活跃线程首先执行加法并将结果写入它们自己的私有临时变量。因此，XY位置中的旧值都不会被覆盖。第14行的__syncthreads()屏障确保所有活跃线程在任何线程可以向前移动并执行写入之前完成了对旧XY值的读取。因此，第16行的语句可以安全地覆盖XY位置。

更新后的XY位置可能被另一个活跃线程使用的原因是因为Kogge-Stone方法在归约树之间重用部分和以减少计算复杂性。我们将在11.3节中进一步研究这一点。读者可能会想知道为什么第10章“归约和最小化发散”中的归约树内核不需要使用临时变量和额外的__syncthreads()。答案是因为在这些归约树内核中没有由写后读危险引起的竞态条件。这是因为在一次迭代中，活跃线程写入的元素不会被同一迭代中的任何其他活跃线程读取。通过检查图10.7和10.8，这一点应该是显而易见的。例如，在图10.8中，每个活跃线程从它自己的位置（input[threadIdx.x]）和一个距离为步长的右侧位置（input[threadIdx.x+stride]）获取输入。在任何给定的迭代中，没有活跃线程会更新任何步长距离位置。因此，所有活跃线程总是能够读取它们各自的input[threadIdx.x]的旧值。由于线程内的执行始终是顺序的，每个线程在将新值写入位置之前，总是能够读取input[threadIdx.x]中的旧值。读者应该验证图10.7中是否具有相同的属性。

如果我们想避免在每次迭代中都有第二个屏障同步，另一种克服竞态条件的方法是使用单独的输入和输出数组。如果使用单独的数组，正在写入的位置与正在读取的位置不同，因此不再有任何潜在的写后读竞态条件。这种方法将需要有一个共享内存缓冲区而不是两个。开始时，我们从全局内存加载到第一个缓冲区。在第一次迭代中，我们从第一个缓冲区读取并写入到第二个缓冲区。迭代结束后，第二个缓冲区有最新的结果，第一个缓冲区的结果不再需要。因此，在第二次迭代中，我们从第二个缓冲区读取并写入到第一个缓冲区。按照相同的逻辑，在第三次迭代中，我们从第一个缓冲区读取并写入到第二个缓冲区。我们继续交替输入/输出缓冲区，直到迭代完成。这种优化被称为双缓冲。双缓冲通常在并行编程中使用，作为克服写后读竞态条件的一种方式。我们将这种优化的实现留给读者作为练习。

此外，如图11.2所示，XY数组中较小位置的操作比较大位置的操作更早结束（参见if语句条件）。这将在步长值较小时，在第一个warp（波次）中引起一定程度的控制发散。请注意，相邻线程倾向于执行相同数量的迭代。对于大的块大小，发散的影响应该相当有限，因为发散只在第一个warp中出现。详细的分析留给读者作为练习。

尽管我们只展示了一个包容性扫描内核，但我们很容易将包容性扫描内核转换为独占性扫描内核。回想一下，独占性扫描等同于将所有元素向右移动一个位置的包容性扫描，并将元素0填充为单位值。这在图11.4中有所说明。请注意，唯一的真正区别是图上方元素的对齐方式。所有标记框都已更新以反映新的对齐方式。所有的迭代操作保持不变。

![image-20240829162446334](imgs/image-20240829162446334.png)

我们现在可以轻松地将图11.3中的内核转换为独占性扫描内核。我们需要进行的唯一修改是在以下代码中所示，将0加载到XY[0]中，将X[i-1]加载到XY[threadIdx.x]中。

![image-20240829162639728](imgs/image-20240829162639728.png)

通过替换图11.3中的第04到08行的这四行代码，我们将包容性扫描内核转换为独占性扫描内核。我们留给读者作为练习.

## 11.3 Speed and work efficiency consideration

在分析并行算法时，一个重要的考虑因素是工作效率。 算法的工作效率指的是算法执行的工作与计算所需的最小工作量接近的程度。例如，扫描操作所需的最小加法次数是 N−1次，或者说是 O(N)，这是顺序算法执行的加法次数。然而，正如我们在第11.2节开头看到的那样，朴素的并行算法执行了 N\*(N−1)/2 次加法，或者说是 $O(N^2)$，这比顺序算法大得多。因此，朴素的并行算法不是工作高效的。

现在我们分析图11.3中Kogge-Stone核心的工作效率，专注于单个线程块的工作。所有线程都迭代到 $log⁡_2N$步，其中N是SECTION_SIZE。在每次迭代中，非活动线程的数量等于步长大小。因此，我们可以计算出算法完成的工作量（图8.1中for循环的一次迭代，由加法运算表示）为

![image-20240830163136282](imgs/image-20240830163136282.png)

每个项的第一部分与步长无关，其求和结果为 $N×log⁡_2(N)$。第二部分是一个熟悉的等比数列，求和结果为 N−1。因此，完成的总工作量是 ![image-20240830163351082](imgs/image-20240830163351082.png)

好消息是，Kogge-Stone方法的计算复杂度是 $O(N \log_2(N)) $，这比执行所有输出元素的完整归约树的朴素方法的 $O(N^2) $ 复杂度要好。坏消息是，Kogge-Stone算法仍然不如顺序算法工作效率高。即使对于中等大小的区段，图11.3中的核比顺序算法完成的工作要多得多。在512个元素的情况下，核完成的工作大约是顺序代码的八倍。随着N变大，这个比例会增加。

尽管Kogge-Stone算法比顺序算法执行更多的计算，但由于并行执行，它在较少的步骤中完成了这些计算。顺序代码的for循环执行N次迭代。对于核代码，每个线程的for循环执行多达 $log_2(N) $ 次迭代，这定义了执行核所需的最小步骤数。如果有无限的执行资源，核代码相对于顺序代码的步骤数减少大约是 $N/\log_2(N) $。对于N=512，步骤数的减少大约是 $ 512/9 \approx 56.93 $。

在实际的CUDA GPU设备中，Kogge-Stone核心完成的工作量超过了理论的 $ N \times \log_2(N) - (N - 1) $。这是因为我们使用了N个线程。虽然许多线程在for循环的执行过程中停止参与，但它们中的一些仍然消耗执行资源，直到整个warp完成执行。实际上，Kogge-Stone消耗的执行资源量更接近于 $ N \times \log_2(N) $。

我们将使用计算步骤的概念作为比较扫描算法的近似指标。顺序扫描大约需要N步来处理N个输入元素。例如，顺序扫描大约需要1024步来处理1024个输入元素。在CUDA设备中有P个执行单元的情况下，我们可以预期Kogge-Stone核心将执行 $(N \times \log_2(N)) / P $步。如果P等于N，也就是说，如果我们有足够的执行单元来并行处理所有输入元素，那么我们就需要 $log_2(N) $ 步，正如我们之前看到的那样。然而，P可能小于N。例如，如果我们使用1024个线程和32个执行单元来处理1024个输入元素，核心可能需要 $(1024 \times 10) / 32 = 320 $步。在这种情况下，我们期望实现步骤数减少了 $ 1024 / 320 = 3.23 $倍

Kogge-Stone核心相对于顺序代码所做的额外工作在两个方面存在问题。首先，执行并行核心的硬件使用效率要低得多。如果硬件没有足够的资源（即，如果P很小），并行算法最终可能需要比顺序算法更多的步骤。因此，该并行算法将会更慢。其次，所有额外的工作消耗了额外的能量。这使得核心不太适合于电源受限的环境，例如移动应用。

Kogge-Stone核心的优势在于，当有足够的硬件资源时，它可以实现非常好的执行速度。它通常用于计算具有适度数量元素的部分的扫描结果，例如512或1024。当然，这假设GPU能够提供足够的硬件资源，并利用额外的并行性来容忍延迟。正如我们所看到的，它的执行具有非常有限的控制分歧量。在较新的GPU架构代中，其计算可以有效地通过warp内的shuffle指令执行。我们将在本章后面看到，它是现代高速并行扫描算法的重要组成部分。

## 11.4 Parallel scan with the Brent-Kung algorithm

图11.3中的Kogge-Stone核心在概念上很简单，但对于一些实际应用来说，其工作效率相当低。仅仅通过检查图11.2和11.4，我们就可以看到有进一步共享一些中间结果的潜在机会。然而，为了允许跨多个线程进行更多的共享，我们需要策略性地计算中间结果并将其分发到不同的线程，这可能需要额外的计算步骤。

我们知道，为一组值生成总和值的最快并行方式是归约树。有足够的执行单元的情况下，归约树可以在 $log_2(N) $时间单位内为N个值生成总和。树还可以生成一些子总和，这些子总和可以用于计算一些扫描输出值。这一观察结果被用作Kogge-Stone加法器设计的依据，也构成了Brent-Kung加法器设计的基础（Brent & Kung, 1979）。Brent-Kung加法器设计也可以用来实现具有更好工作效率的并行扫描算法。

图11.5展示了基于Brent-Kung加法器设计的并行包容性扫描算法的步骤。在图11.5的上半部分，我们在四个步骤中生成了所有16个元素的总和。我们使用了生成总和所需的最小操作数。在第一步中，只有XY[i]中的奇数元素将更新为 \( XY[i-1] + XY[i] \)。在第二步中，只有索引形式为 \( 4k - 1 \) 的XY元素，即图11.5中的3、7、11和15，将被更新。在第三步中，只有索引形式为 \( 8k - 1 \) 的XY元素，即7和15，将被更新。最后，在第四步中，只有XY[15]被更新。执行的总操作数是 \( 8 + 4 + 2 + 1 = 15 \)。一般来说，对于N个元素的扫描部分，我们将为这个归约阶段做 $ (N/2) + (N/4) + ... + 2 + 1 = N^2 - 1 $次操作。

![image-20240830164439335](imgs/image-20240830164439335.png)
...
我们的下一个并行模式是稀疏矩阵计算。在稀疏矩阵中，大多数元素都是零。存储和处理这些零元素在内存容量、内存带宽、时间和能量方面都是浪费的。许多重要的现实世界问题涉及稀疏矩阵计算。由于这些问题的重要性，已经提出并广泛使用了几种稀疏矩阵存储格式及其相应的处理方法。所有这些方法都采用了某种类型的压缩技术，以避免存储或处理零元素，代价是引入了一定程度的数据表示不规则性。不幸的是，这种不规则性可能导致内存带宽的低效利用、控制流发散和并行计算中的负载不平衡。因此，找到压缩和规范化之间的良好平衡非常重要。一些存储格式在高度不规则性下实现了更高程度的压缩。其他则在保持表示更规则的同时实现了更适度的压缩水平。众所周知，使用每种存储格式的并行计算的相对性能严重依赖于稀疏矩阵中非零元素的分布。了解稀疏矩阵存储格式及其相应并行算法的丰富工作为解决相关问题中的压缩和规范化挑战为并行程序员提供了重要的背景。
...
机器学习这一术语由 IBM 的亚瑟・塞缪尔（Arthur Samuel）于 1959 年创造（塞缪尔，1959），它是计算机科学的一个领域，研究从数据中学习应用逻辑的方法，而非设计显式算法。机器学习在那些设计显式算法不可行的计算任务中最为成功，主要是因为在设计此类显式算法时缺乏足够的知识。也就是说，人们可以给出在各种情况下应该发生什么的示例，但却没有针对所有可能输入做出此类决策的通用规则。例如，机器学习为自动语音识别、计算机视觉、自然语言处理和推荐系统等应用领域的近期改进做出了贡献。在这些应用领域中，人们可以提供许多输入示例以及每个输入对应的输出，但没有一种算法能够正确处理所有可能的输入。

通过机器学习创建的各种应用逻辑可以根据它们所执行的任务类型进行分类。机器学习任务种类繁多。在此，我们从大量任务中列举几个：

1. **分类**：确定一个输入属于 k 个类别中的哪一个。例如物体识别，比如判断照片中展示的是哪种食物。
2. **回归**：根据一些输入预测一个数值。例如预测下一个交易日结束时的股票价格。
3. **转录**：将非结构化数据转换为文本形式。例如光学字符识别。
4. **翻译**：将一种语言的符号序列转换为另一种语言的符号序列。例如从英语翻译成中文。
5. **嵌入**：将一个输入转换为一个向量，同时保留实体之间的关系。例如将一个自然语言句子转换为一个多维向量。

读者可参考大量关于机器学习各种任务的数学背景和实际解决方案的文献。本章的目的是介绍神经网络方法用于分类任务中所涉及的计算内核。对这些内核的具体理解将使读者能够理解并开发用于其他机器学习任务的深度学习方法的内核。因此，在本节中，我们将详细介绍分类任务，以建立理解神经网络所需的背景知识。从数学上讲，分类器是一个函数$f$，它将一个输入映射到$k$个类别或标签：

 $f: \mathbb{R}^n \to \{1, 2, \ldots, k\}$

函数$f$由参数$\theta$进行参数化，它将输入向量$\mathbf{x}$映射到数值编码$\mathbf{y}$，即

 $y = f(\mathbf{x}, \theta)$ 

参数$\theta$通常被称为模型。它封装了从数据中学习到的权重。用一个具体的例子可以最好地说明$\theta$的这个定义。让我们考虑一个称为感知机的线性分类器（罗森布拉特，1957）：$y = \text{sign}(\mathbf{W} \cdot \mathbf{x} + b)$，其中$\mathbf{W}$是与$\mathbf{x}$长度相同的权重向量，$b$是偏置常数。符号函数$\text{sign}$如果输入为正，则返回值$1$；如果输入为$0$，则返回$0$；如果输入为负，则返回$-1$。也就是说，符号函数作为分类器激活，将输入值映射到三个类别：$\{-1, 0, 1\}$；因此它通常被称为激活函数。激活函数将非线性引入到感知机原本的线性函数中。在这种情况下，模型$\theta$是向量$\mathbf{W}$和常数$b$的组合。模型的结构是输入的线性表达式，其中系数元素是$\mathbf{W}$中的元素，常数是$b$。 

![image-20250208164956258](imgs/image-20250208164956258.png)

图16.1展示了一个感知机示例，其中每个输入是一个二维（2D）向量$(x_1, x_2)$。线性感知机的模型$\theta$由权重向量$(w_1, w_2)$和偏置常数$b$组成。如图16.1所示，线性表达式$w_1x_1 + w_2x_2 + b$在$x_1 - x_2$空间中定义了一条直线，该直线将空间分为两部分：所有使表达式大于零的点所在的部分，以及所有使表达式小于零的点所在的部分。直线上的所有点使表达式等于$0$。

 直观地说，给定$(w_1, w_2)$和$b$值的组合，我们可以在$(x_1, x_2)$空间中绘制一条直线，如图16.1所示。例如，对于一个模型为$(w_1, w_2) = (2, 3)$且$b = -6$的感知机，我们可以通过连接与$x_1$轴的交点$((\frac{-b}{w_1}, 0) = (3, 0))$和与$x_2$轴的交点$((0, \frac{-b}{w_2}) = (0, 2))$轻松绘制一条直线。这样绘制的直线对应方程$2x_1 + 3x_2 - 6 = 0$。通过此绘图，我们可以轻松直观地看到输入点的分类结果：直线上方的任何点（在图16.1中显示为蓝色点）被分类为类别$1$，直线上的任何点被分类为类别$0$，直线下方的任何点（在图16.1中显示为橙色点）被分类为类别$-1$。 计算输入的分类的过程通常称为分类器的推断。在感知机的情况下，我们只需将输入坐标值代入$y = \text{sign}(W \cdot x + b)$。在我们的示例中，如果输入点是$(5, -1)$，我们可以通过将其坐标代入感知机函数来进行推断：

 $y = \text{sign}(2 * 5 + 3 * (-1) + 6) = \text{sign}(13) = 1$

因此，$(5, -1)$被分类为类别$1$，即它属于蓝色点的范畴。 

#### 多层分类器 

当存在一种方法可以绘制超平面（即在二维空间中的直线，在三维空间中的平面），将空间划分为不同区域，从而定义数据点的每个类别时，线性分类器是有用的。理想情况下，每个类别的数据点应该恰好占据一个这样的区域。例如，在一个二维、二分类的分类器中，我们需要能够绘制一条直线，将一个类别的点与另一个类别的点分开。不幸的是，这并不总是可行的。

 考虑图16.2中的分类器。假设所有输入的坐标都落在$[0, 1]$范围内。分类器应该将$x_1$和$x_2$值都大于$0.5$的所有点（即落在定义域右上角的点）分类为类别$1$，将其余点分类为类别$-1$。这个分类器可以近似地用如图16.2(A)所示的直线来实现。例如，直线$2x_1 + 2x_2 - 3 = 0$可以正确地对大多数点进行分类。然而，一些$x_1$和$x_2$都大于$0.5$但它们的和小于$1.5$的橙色点，例如$(0.55, 0.65)$，会被错误地分类为类别$-1$（蓝色）。这是因为任何直线必然要么会切掉右上角区域的一部分，要么会包含定义域的一部分不需要的点。没有一条直线能够正确地对所有可能的输入进行分类。 

![image-20250208165344692](imgs/image-20250208165344692.png)

多层感知机（MLP）允许使用多条直线来实现更复杂的分类模式。在多层感知机中，每一层由一个或多个感知机组成。一层中感知机的输出是下一层感知机的输入。一个有趣且有用的性质是，虽然第一层的输入有无限多个可能的值，但第一层的输出使得第二层的输入只能有适度数量的可能值。例如，如果将图16.1中的感知机用作第一层，其输出将被限制在$\{-1, 0, 1\}$。 

图16.2(B)展示了一个两层感知机，它可以精确地实现所需的分类模式。第一层由两个感知机组成。第一个感知机，$y_1 = \text{sign}(x_1 - 0.5)$，将所有$x_1$坐标大于$0.5$的点分类为类别$1$，即输出值为$1$。其余的点被分类为类别$-1$或类别$0$。第一层的第二个分类器，$y_2 = \text{sign}(x_2 - 0.5)$，将所有$x_2$坐标大于$0.5$的点分类为类别$1$。其余的点被分类为类别$-1$。

 因此，第一层的输出$(y_1, y_2)$只能是以下九种可能之一：$(-1, -1), (-1, 0), (-1, 1), (0, -1), (0, 0), (0, 1), (1, -1), (1, 0), (1, 1)$。也就是说，第二层只有九种可能的输入对值。在这九种可能性中，$(1, 1)$是特殊的。橙色类别的所有原始输入点都被第一层映射到$(1, 1)$。因此，我们可以在第二层使用一个简单的感知机，在$y_1 - y_2$空间中$(1, 1)$和其他八个可能的点之间绘制一条直线，如图16.2B所示。这可以通过直线$2y_1 + 2y_2 - 3 = 0$或其他许多与之略有不同的直线来实现。 

让我们使用在图16.2A中被单层感知机错误分类的$(0.55, 0.65)$输入。当由两层感知机处理时，图16.2B中第一层的上层感知机生成$y_1 = 1$，下层感知机生成$y_2 = 1$。基于这些输入值，第二层的感知机生成$z = 1$，这是对$(0.55, 0.65)$的正确分类。 

请注意，两层感知机仍然有显著的局限性。例如，假设我们需要构建一个感知机来对图16.3A中所示的输入点进行分类。橙色输入点的值可能会导致第二层的输入值为$(-1, -1)$或$(1, 1)$。我们发现，没有办法绘制一条直线来正确地对第二层的点进行分类。我们在图16.2B中展示了，我们可以通过在第二层添加另一个感知机来添加另一条直线。其函数可以是$z_2 = \text{sign}(-2y_1 - 2y_2 - 3)$或其细微变化形式。读者应该验证，图16.3B中的所有蓝色点在$z_1 - z_2$空间中都会被映射到$(-1, -1)$。而在$y_1 - y_2$空间中的$(1, 1)$和$(-1, -1)$在$z_1 - z_2$空间中会被映射到$(1, -1)$和$(-1, 1)$。现在我们可以绘制一条直线$z_1 + z_2 + 1 = 0$或其细微变化形式来正确地对这些点进行分类，如图16.3C所示。显然，如果我们需要将输入定义域划分为更多区域，我们可能需要更多层来进行正确分类。 

![image-20250208165927342](imgs/image-20250208165927342.png)

图16.2B中的第一层是全连接层的一个小例子，其中每个输出（即$y_1, y_2$）都是每个输入（即$x_1, x_2$）的函数。一般来说，在全连接层中，$m$个输出中的每一个都是所有$n$个输入的函数。全连接层的所有权重形成一个$m \times n$的权重矩阵$W$，其中$m$行中的每一行都是一个权重向量（大小为$n$个元素），应用于输入向量（大小为$n$个元素）以产生$m$个输出中的一个。因此，从全连接层的输入计算所有输出的过程是一个矩阵 - 向量乘法。正如我们将看到的，全连接层是许多类型神经网络的核心组件，我们将进一步研究它们的GPU实现。 

当$m$和$n$很大时，全连接层的成本会变得极高。主要原因是全连接层需要一个$m \times n$的权重矩阵。例如，在图像识别应用中，$n$是输入图像中的像素数量，$m$是需要对输入像素进行分类的类别数量。在这种情况下，对于高分辨率图像，$n$可达数百万，而$m$可能有数百甚至更多，具体取决于需要识别的物体种类。此外，图像中的物体可能具有不同的比例和方向；可能需要多个分类器来处理这些变化。为所有这些分类器提供所有输入既昂贵又可能浪费。 

卷积层通过减少每个分类器所接收的输入数量，并在分类器之间共享相同的权重，降低了全连接层的成本。在卷积层中，每个分类器仅取输入图像的一个小块，并根据权重对小块中的像素执行卷积操作。输出被称为输出特征图，因为输出中的每个像素都是一个分类器的激活结果。在分类器之间共享权重使得卷积层能够拥有大量的分类器，即较大的$m$值，而无需过多的权重。在计算上，这可以实现为二维卷积。然而，这种方法实际上是将相同的分类器应用于图像的不同部分。可以将不同的权重集应用于相同的输入，并生成多个输出特征图，这一点我们将在本章后面看到。 

#### 训练模型 

到目前为止，我们一直假设分类器使用的模型参数以某种方式是可用的。现在我们转向训练，即使用数据来确定模型参数$\theta$的值的过程，这些参数包括权重$(w_1, w_2)$和偏置$b$。为简单起见，我们将假设是监督训练，在这种训练中，输入数据带有标记的输出值，用于确定权重和偏置值。其他训练方式，如半监督学习和强化学习，也已经被开发出来以减少对标记数据的依赖。读者可参考相关文献以了解在这种情况下如何进行训练。

 #### 误差函数

 一般来说，训练将模型参数视为未知变量，并在给定带标记的输入数据的情况下解决一个逆问题。在图16.1的感知机示例中，每个数据点都带有期望的分类结果：$-1$、$0$或$1$。训练过程通常从对$(w_1, w_2)$和$b$的值进行初始猜测开始，然后对输入数据进行推断并生成分类结果。这些分类结果与标记进行比较。定义一个误差函数，有时也称为代价函数，用于量化每个数据点的分类结果与相应标记之间的差异。例如，假设$y$是分类输出类别，$t$是标记。以下是一个误差函数的示例：

 $E = \frac{(y - t)^2}{2}$

这个误差函数有一个很好的性质：只要$y$和$t$的值之间存在任何差异，无论是正还是负，误差值总是正的。如果我们需要对许多输入数据点的误差进行求和，正的和负的差异都会对总和有贡献，而不会相互抵消。人们也可以将误差定义为差异的绝对值，还有许多其他的定义方式。正如我们将看到的，系数$\frac{1}{2}$简化了求解模型参数时所涉及的计算。  
...
到目前为止，我们一直专注于并行编程中的实用知识，包括CUDA编程接口特性、GPU架构、性能优化技术、并行模式以及应用案例研究。在本章中，我们将话题转向一些更抽象的概念。我们将并行编程概括为一个计算思维过程，即设计或选择并行算法，并将领域问题分解为定义明确且相互协调的工作单元，每个工作单元都可以由所选算法高效执行。 具有强大计算思维能力的程序员不仅会分析领域问题的结构，还会对其进行改造：哪些部分本质上是串行的，哪些部分适合高性能并行执行，以及在将部分从前者类别转移到后者类别时所涉及的特定领域权衡。借助良好的算法选择和问题分解，程序员可以在并行性、工作效率和资源消耗之间取得适当的平衡。 要为具有挑战性的领域问题创建成功的计算解决方案，通常需要将领域知识与计算思维能力紧密结合。本章将让读者对并行编程和一般的计算思维有更深入的了解。 

### 19.1 并行计算的目标 

在讨论并行编程的基本概念之前，我们首先回顾一下人们追求并行计算的三个主要原因，这很重要。第一个目标是在更短的时间内解决给定问题。例如，一家投资公司可能需要在交易时间结束后，对其所有投资组合运行一个投资组合情景风险分析软件包。在顺序计算机上，这样的分析可能需要200个小时。然而，投资组合管理流程可能要求在4小时内基于分析结果完成决策所需的分析。使用并行计算可能会加快分析速度，使其在规定的时间窗口内完成。 

使用并行计算的第二个目标是在给定的时间量内解决更大的问题。在我们的金融投资组合分析示例中，投资公司可能能够使用顺序计算在给定的时间窗口内，对其当前投资组合运行投资组合情景风险分析。然而，该公司计划扩大其投资组合中的持有数量。扩大后的问题规模将导致顺序分析超出允许的时间窗口。并行计算可以减少运行更大问题规模的时间，有助于适应投资组合的计划扩展。

 使用并行计算的第三个目标是在给定的时间量内为给定问题实现更好的解决方案。投资公司可能一直在其投资组合情景风险分析中使用近似模型。使用更准确的模型可能会增加计算复杂度，并使在顺序计算机上的运行时间超出允许的时间窗口。例如，更准确的模型可能需要考虑更多类型的风险因素，使用更复杂的公式。减少更准确模型运行时间的并行计算，可能会在允许的时间窗口内完成分析。 

在实践中，并行计算可能是由这三个目标的组合所驱动的。 

从我们的讨论中应该清楚，并行计算主要是由速度提升所驱动的。第一个目标是通过在当前问题规模上更快地运行现有模型来实现的。第二个目标是通过在更大的问题规模上更快地运行现有模型来实现的。第三个目标是通过在当前问题规模上更快地运行更复杂的模型来实现的。显然，并行计算带来的速度提升可用于实现这些目标的组合。例如，并行计算可以减少在更大问题规模上运行更复杂模型的运行时间。

从我们的讨论中还应该清楚，适合并行计算的应用程序通常涉及较大的问题规模和较高的复杂性。也就是说，这些应用程序处理大量数据，在每次迭代中需要大量计算，并且 / 或者对数据执行许多次迭代。那些不涉及较大问题规模或不产生高建模复杂性的应用程序往往能在短时间内完成，并且没有太多提升速度的动力。

一个实际的应用程序通常由多个协同工作的模块组成。图 19.1 展示了一个分子动力学应用程序的主要模块概述。对于系统中的每个原子，该应用程序需要计算施加在该原子上的各种力，如振动力、转动力和非键力。每种力都用不同的方法计算。在高层次上，程序员需要决定如何组织工作。请注意，这些模块之间的工作量可能会有很大差异。非键力计算通常涉及许多原子之间的相互作用，比振动力和转动力的计算量要大得多。因此，这些模块往往会在力数据结构上分别进行处理。

程序员需要决定每个处理步骤是否值得在 CUDA 设备上实现。例如，程序员可能会认为振动力和转动力的计算工作量不足以支持在 GPU 设备上执行。这样的决定会导致一个 CUDA 程序启动一个内核来计算所有网格点的非键力场，同时继续在主机上计算网格点的振动力和转动力。更新原子位置和速度的模块也可能在主机上运行。它首先将来自设备的振动力和转动力与主机上的非键力相结合。然后，它使用组合后的力来计算新的原子位置和速度。

![image-20250208155023265](imgs/image-20250208155023265.png)

设备完成的那部分工作最终将决定通过并行化实现的应用程序级加速比。例如，假设非键力计算占原始顺序执行时间的95%，并且使用GPU将其加速100倍。进一步假设应用程序的其余部分保持不变且没有获得加速。应用程序级加速比为$1/(5\% + 95\%/100) = 1/(5\% + 0.95\%) = 1/(5.95\%) = 17\times$ 。在主机和CUDA设备的执行可以重叠的情况下，并行部分的执行时间将完全隐藏在主机执行时间中。应用程序级加速比将为$1/(5\%) = 20\times$ 。这是阿姆达尔定律的一个演示：并行计算带来的应用程序加速比受到应用程序顺序部分的限制。在这种情况下，尽管应用程序的顺序部分相当小（5%），但它将应用程序级加速比限制为20倍，尽管非键力计算的加速比为100倍，并且主机执行顺序部分的执行时间被隐藏。这个例子说明了加速大型应用程序的一个主要挑战：对于不值得在CUDA设备上并行执行的小活动，其累积执行时间可能会成为用户看到的加速比的限制因素。我们在第17章“迭代磁共振成像重建”的MRI应用研究中看到了这种现象，其中共轭梯度（CG）计算尽管只占原始应用程序执行时间的约1%，但却成为了加速比的限制因素。

 阿姆达尔定律常常推动任务级并行化。尽管其中一些较小的活动不值得进行细粒度的大规模并行执行，但当数据集足够大时，并行执行这些活动可能是可取的。这可以通过使用多核主机并行执行此类任务来实现。或者，我们可以尝试同时支持多个小内核，每个内核对应一个任务。CUDA设备支持使用流进行任务并行化，这将在第20章“异构计算集群编程”中讨论。 

减少顺序任务影响的另一种方法是分层利用数据并行性。例如，在消息传递接口（MPI，2009）的实现中，分子动力学应用程序通常会将大量的空间网格块及其相关原子分配给联网计算集群的节点。通过使用每个节点的主机为这些原子的每个块计算振动力和转动力，我们可以利用多个 CPU 并实现一定程度的加速。每个节点可以使用 CUDA 设备以更高的加速比计算非键力。节点之间需要交换数据，以处理跨越块的力以及跨越块边界移动的原子。我们将在第 20 章 “异构计算集群编程” 中讨论 MPI - CUDA 联合编程的更多细节。这里的重点是，在应用程序中，MPI 和 CUDA 可以以互补、分层的方式使用，以便在处理大型数据集时共同实现更高的速度。

并行编程的过程通常可以分为三个步骤：算法选择、问题分解以及性能优化和调优。上几章以及第 6 章 “性能考量” 已经对最后一个步骤进行了一般性的讨论。在本章接下来的两个部分中，我们将全面且深入地讨论前两个步骤。

### 19.2 算法选择

算法是一种分步过程，其中每一步都有精确的表述，并且可由计算机执行。算法必须具备三个基本属性：确定性、有效可计算性和有限性。确定性是指每一步都有精确表述的概念；对于要执行的操作不存在歧义。有效可计算性是指每一步都能由计算机执行这一事实。有限性意味着算法必须保证终止。

对于给定的问题，通常可以想出多种算法来解决它。有些算法所需的计算步骤比其他算法少（即具有较低的算法复杂度），有些算法展现出比其他算法更高程度的并行执行能力，有些算法比其他算法具有更广泛的适用性，还有些算法在数值稳定性方面比其他算法更好。不幸的是，往往没有一个单一的算法在所有这些方面都优于其他算法。并行程序员通常需要为给定的硬件系统选择一种在各方面能达到最佳折衷的算法。

在本书的多个章节中，我们已经看到了评估不同算法在计算方面权衡的示例。在第 11 章 “前缀和（扫描）” 中关于前缀和计算的内容里，我们比较了两种用于执行并行前缀和的不同算法，即科格 - 布伦特（Kogge - Brent）算法和布伦特 - 孔（Brent - Kung）算法。我们的分析表明，布伦特 - 孔算法具有较低的算法复杂度。它在执行相同计算时所需的操作更少，这使其效率更高。然而，我们也表明科格 - 布伦特算法展现出更多的并行性。与布伦特 - 孔算法相比，它能在更少的迭代次数内完成计算。算法复杂度与算法所展现的并行量之间的这种权衡，是并行程序员会遇到的经典权衡。最佳算法通常取决于目标并行硬件的特性，以及通过结合两种并行算法或通过线程合并将并行算法与低复杂度顺序算法相结合的混合方法，在多大程度上可以减轻算法较高复杂度的影响。

在第 13 章 “排序” 中关于排序模式的内容里，我们比较了两种用于执行并行排序的不同算法，即基数排序和归并排序。基数排序可以比归并排序实现更低的算法复杂度，因为它是一种非比较排序算法。它也非常适合并行化。然而，基数排序一般不具有广泛的适用性，因为它只能与某些类型的键一起使用。归并排序是一种基于比较的排序算法，具有更广泛的适用性，可以与任何具有明确定义比较运算符的键类型一起使用。通用性和并行执行效率之间的权衡，是并行程序员在选择并行算法时遇到的另一种权衡。

在第 18 章 “静电势图” 中关于静电势图计算的内容里，我们比较了两种用于执行该计算的不同算法，即直接库仑求和（DCS）和截断求和（罗德里格斯等人，2008）。这两种方法展现出相同程度的并行性；然而，它们呈现出算法复杂度和准确性之间的经典权衡。在第 18 章 “静电势图” 中，我们表明截断求和算法可以通过牺牲少量准确性来显著提高网格计算的执行效率。这种技术所解决的挑战是，对于完全精确的方法（如 DCS），计算量会随着体积的平方而增加。对于大规模并行设备来说，即使是这样的计算量也会使计算时间过长。截断求和方法基于这样一种观察，即许多网格计算问题是基于物理定律的，在这些定律中，来自远离网格点的粒子或样本的数值贡献很小，可以集体视为隐含的，从而大大降低了算法复杂度。虽然算法复杂度和准确性之间的这种权衡并非并行编程所独有，在顺序实现中也会遇到，但它可能会给并行程序员带来额外的挑战。我们在截断求和算法的示例中展示了这些额外的挑战，这也是本节的重点内容。

在顺序计算中，一个简单的截断算法一次处理一个原子。对于每个原子，该算法会遍历位于该原子坐标半径范围内的网格点。这是一个直接的过程，因为网格点在一个数组中，可以很容易地根据它们的坐标进行索引。然而，这个简单的过程并不容易进行并行执行。原因是基于原子中心的分解方式效果不佳，因为其内存访问具有分散性。因此，我们需要使用一种基于网格点中心分解的截断分箱算法。

在这种网格点中心分解中：每个线程计算一个网格点的能量值。该算法的关键思想是首先根据输入原子的坐标将它们排序到各个箱中。每个箱对应于网格空间中的一个盒子，并且包含所有坐标落在该盒子内的原子。我们将一个网格点的箱的 “邻域” 定义为包含所有可能对该网格点能量值有贡献的原子的箱的集合。在第 18 章 “静电势图” 中，我们描述了一种为所有网格点管理邻域箱的高效方法。在这个算法中，所有线程块在它们各自的邻域箱中迭代，同时所有线程在一个块中可以遍历这些箱中的原子，但要各自决定每个原子是否在其截断半径内。

分箱的一个微妙问题是，箱最终可能包含不同数量的原子。有些箱可能有很多原子，而有些箱可能根本没有原子。一个众所周知的解决方案是将箱的大小设置在一个合理的水平，通常远小于一个箱中可能的最大原子数。分箱过程会维护一个溢出列表。当原子被排序到箱中时，如果一个原子所属的箱已满，该原子就会被添加到溢出列表中。在设备完成执行用于计算静电势图的截断求和内核后，溢出列表中的原子需要进行处理，以完成缺失的贡献计算。

图19.2展示了各种静电势图算法和实现的可扩展性与性能的对比。请注意，CPU - SSE3曲线基于顺序截断求和算法。对于体积较小（约1000 $\mathring{A}^3$）的图，主机（具有SSE的CPU）的执行速度比直接库仑求和（DCS）快，如图19.2所示。这是因为对于如此小的体积，没有足够的工作来充分利用CUDA设备。然而，对于中等体积（在2000到500,000 $\mathring{A}^3$ 之间），直接求和内核由于其大规模并行性，性能明显优于主机。然而，正如我们预期的那样，当体积达到约1,000,000 $\mathring{A}^3$ 时，直接求和内核的扩展性比顺序算法（CPU）更差！这是因为DCS内核的算法复杂度高于顺序截断算法，因此内核完成的工作量增长速度远快于顺序算法。对于体积大于1,000,000 $\mathring{A}^3$ 的情况，工作量如此之大，以至于耗尽了硬件执行资源。 

![image-20250208161303307](imgs/image-20250208161303307.png)

图19.2还展示了截断求和算法的三种基于分箱实现的运行时间。SmallBin实现对应于第18章“静电势图”中讨论的邻域箱列表方法，它允许块运行相同的内核来处理无静电影响的邻域原子。SmallBin实现确实在将原子从全局内存加载到共享内存时会产生更多的指令开销。对于中等体积，整个系统中的原子数量有限。检查较少数量的原子并不能提供足够的优势来克服额外的指令开销。SmallBin - Overlap实现将顺序溢出原子处理与下一次内核执行重叠。与SmallBin实现相比，它在运行时间上提供了轻微但明显的改进。SmallBin - Overlap实现相对于高效实现的顺序CPU - SSE截断求和实现实现了17倍的加速，并且对于较大体积保持了相同的可扩展性。 

### 19.3 问题分解

在选择了合适的算法之后，对于要通过并行计算解决的问题，必须以一种能够将其分解为可以同时安全解决的子问题的方式来表述。在这样的表述和分解下，程序员编写代码并组织数据以并发地解决这些子问题。在大型计算问题中寻找并行性在概念上通常很简单，但在实践中可能具有挑战性。关键是确定每个并行执行单元要执行的工作，以便问题中固有的并行性得到充分利用。

为并行执行分解问题的两种最常见策略是输出中心分解和输入中心分解。顾名思义，输出中心分解分配线程来处理输出数据的不同单元，而输入中心分解分配线程来并行处理输入数据的不同单元。这些分解策略如图 19.3 所示。虽然两种分解策略会导致相同的执行结果，但它们在给定的硬件系统中可能表现出非常不同的性能。输出中心分解通常表现出收集内存访问行为，即每个线程收集或汇集输入值的效果以形成一个输出值。图 19.3A 展示了收集访问行为。基于收集的访问模式在 CUDA 设备中通常更可取，因为线程可以在其私有寄存器中累积结果。此外，多个线程可以共享输入值，并且可以有效地使用常量内存缓存或共享内存来节省全局内存带宽。

相比之下，输入中心分解通常表现出分散内存访问行为，即每个线程将一个输入值的效果分散或分配到输出值中。分散行为如图 19.3B 所示。在 CUDA 设备中，基于分散的访问模式通常不太可取，因为多个线程可能会在同一时间更新同一个网格点。网格点必须存储在可以被所有涉及的线程写入的内存中。必须使用原子操作来防止在多个线程同时写入一个输出值时出现竞态条件和值丢失。这些原子操作比输出中心分解中使用的寄存器访问明显要慢。

![image-20250208161908526](imgs/image-20250208161908526.png)

然而，除了收集与分散访问模式之外，在决定输出中心分解、输入中心分解（或其他分解方式）哪种更适合特定应用时，还可能需要考虑其他因素。这些因素包括分解所展现的并行性程度、确定哪些输入数据对哪些输出数据有贡献的难易程度，以及分解所引发的负载平衡情况，而这取决于应用程序。

输入中心分解和输出中心分解之间的区别在第 17 章 “迭代磁共振成像重建” 和第 18 章 “静电势图” 中最为明显，在这两章中，两种策略都得以实现并进行了明确的比较。然而，问题分解是本书所讨论的许多计算中隐含的设计决策。我们重新审视这些计算，以强调在每种情况下所选择的问题分解方式以及它相对于其他分解方式的优势。

图像处理（第 3 章 “多维网格和数据”）、矩阵乘法（第 3、5 和 6 章）、卷积（第 7 章）和模板计算（第 8 章）都是通过输出中心分解来实现并行化的。也就是说，在这些计算中，线程被分配给输出元素（图像像素、矩阵元素或网格点），并对贡献于它们的输入元素进行迭代。或者，输入中心分解会将线程分配给输入元素，并让每个线程对其输入元素所贡献的输出元素进行迭代，并使用原子操作进行更新。在这些情况下，输出中心分解相对于输入中心分解的明显优势在于，它通过使用收集访问模式而非分散访问模式来避免原子操作。另一方面，没有其他因素使得输入中心分解更具优势。有足够多的输出元素来展现高度的并行性，确定每个输出元素所需的输入元素很直接，并且不需要输出元素进行相同数量的工作，所以不存在负载不平衡问题

直方图计算（第 9 章 “并行直方图”）是通过输入中心分解来实现并行化的。也就是说，每个线程被分配给一个输入元素（或元素块），并根据输入值更新输出 bins（区间）。由于多个线程可能会更新同一个输出 bin，所以需要原子操作（分散访问模式）。或者，采用输出中心分解方式，会将线程分配给输出 bins，每个线程对映射到该 bin 的输入进行搜索，并相应地更新 bin。这种分解方式将消除原子操作，因为每个 bin 将由单个线程更新（收集访问模式）。然而，输出中心分解存在两个问题。首先，输出 bins 的数量通常比输入值的数量少得多，所以并行性会大幅降低。其次，线程在不检查输入值的情况下，无法知道哪些输入值映射到其输出 bin，所以每个线程都需要遍历每个输入值，这并不高效。第三，即使线程有某种快速识别哪些输入值映射到其输出 bin 的方法，每个 bin 映射到的输入值数量也会不同，这将导致线程间的负载不平衡。所有这些因素使得输入中心分解对于直方图计算更有利。

合并操作（第 12 章 “合并”）是通过输出中心分解来实现并行化的；也就是说，每个线程被分配给输出数组中的一个元素（或元素块），并执行二分查找以找到对应的输入元素并合并它们。虽然许多其他倾向于输出中心分解的计算是因为收集优于分散的优势，但这一考虑因素对于合并操作并不相关。在合并操作中，每个输出元素仅由一个输入元素贡献，所以输入中心的合并操作不需要使用原子操作。然而，输入中心的合并操作要么会使找到元素之间的映射关系成本更高，要么会因为让每个输入线程负责不同数量的输入元素而导致高负载不平衡。出于这个原因，更倾向于输出中心分解。

对于稀疏矩阵计算（第 14 章 “稀疏矩阵计算”），SpMV/COO 内核使用了输入中心分解，即线程被分配给输入矩阵中的非零元素，并以原子方式更新输出向量中的相应元素（分散访问模式）。其余的内核使用输出中心分解，线程被分配给输出向量元素，并对输入非零元素进行迭代（收集访问模式）。尽管输入中心的 SpMV/COO 内核执行了更多的原子操作，但它在提取并行性和实现更好的负载平衡方面具有其他优势。最终，这种计算的最佳分解方式取决于输入数据集。此外，混合 ELL - COO 格式展示了一个示例，其中混合输出中心和输入中心分解可能是有益的。

对于图遍历（第 15 章 “图遍历”），基于顶点的推送实现和基于边的实现使用了输入中心分解。也就是说，线程被分配给顶点或边，并更新相邻顶点的级别。相比之下，基于顶点的拉取实现使用了输出中心分解，其中每个线程仅更新其被分配到的顶点的级别。在选择不同的分解方式时，分散和收集访问模式之间的权衡不是一个问题，因为对级别值的更新是在顶点之间进行的，不需要原子操作。然而，提取的并行性数量和实现的负载平衡在决定哪种分解方式更有利方面起着重要作用，最佳分解方式最终取决于输入数据集。读者可参考第 15 章 “图遍历” 以获取关于此主题的更详细处理。

迭代磁共振成像重建问题（第 17 章 “迭代磁共振成像重建”）和静电势计算问题（第 18 章 “静电势图”）由于收集模式优于分散模式在避免原子操作方面的优势，倾向于输出中心分解。这些章节已经深入探讨了两种分解策略之间的区别。迭代磁共振成像重建问题（第 17 章 “迭代磁共振成像重建”）处理大量的 k 空间采样数据。每个 k 空间采样数据也多次用于计算其对重建体素数据的贡献。对于高分辨率重建，每个采样数据被使用大量次数。我们表明，在磁共振成像重建中，FhD 问题的良好分解是输出中心分解，它形成子问题，每个子问题使用收集策略计算 FhD 元素的一个输出值。

同样，静电势计算问题（第 18 章 “静电势图”）涉及计算许多输入原子对大量输出网格点处的势能的贡献。一个实际的分子系统模型通常至少涉及数十万个原子和数百万个能量网格点。每个原子的静电电荷信息在计算其对能量网格点的贡献时会被多次使用。我们表明，静电势图计算问题的分解可以是基于原子的（即输入中心的）或基于网格点的（即输出中心的）。在基于原子的分解中，每个线程负责计算一个原子对所有网格点的影响。相比之下，基于网格点的分解使用每个线程来计算所有原子对一个网格点的影响。

事实证明，基于网格点的（即输出中心的）分解更好，因为它使用了更有利的收集访问模式。两种分解方式所展现的并行性数量都是足够的。对于截断求和算法，在基于网格点的分解中，将输入数据映射到输出数据的困难通过使用分箱方法得以克服，而分箱所导致的负载不平衡则通过上一节讨论的 SmallBin - Overlap 实现得以克服。

### 19.4 计算思维

计算思维可以说是并行应用开发中最重要的方面（温，2006）。我们将计算思维定义为根据计算步骤和算法来表述领域问题的思维过程。与任何其他思维过程和问题解决技能一样，计算思维是一门艺术。正如我们在第 1 章 “引言” 中提到的，我们认为计算思维最好通过迭代方法来教授，在这种方法中，学生在实践经验和抽象概念之间来回切换。

关于各种算法、问题分解和优化策略的文献数量庞大，可能难以理解。全面涵盖所有可用技术超出了本书的范围。我们在算法、问题分解和优化的每个步骤中都讨论了大量具有广泛适用性的技术。虽然这些技术是通过 CUDA C 实现来演示的，但它们有助于读者总体上建立计算思维的基础。我们认为，当我们从下往上学习时，人类理解得最好。也就是说，我们首先在特定编程模型的背景下学习概念，这在我们将知识推广到其他编程模型之前为我们提供了坚实的基础。对 CUDA C 实现的深入体验也使我们能够走向成熟，这将帮助我们学习甚至可能与 GPU 无关的并行编程和计算思维概念。

并行程序员要成为高效的计算思考者，需要掌握众多技能。我们将这些基础技能总结如下：

- **计算机体系结构**：内存组织、缓存与局部性、内存带宽、单指令多线程（SIMT）、单程序多数据（SPMD）与单指令多数据（SIMD）执行方式，以及浮点精度与准确性。这些概念对于理解算法、问题分解和优化之间的权衡至关重要。
- **编程语言接口与编译器**：并行执行模型、数据类型、可用内存、同步支持、数组数据布局以及线程粒度转换。为了深入思考数据结构和循环结构的安排以实现更好的性能，需要了解这些概念。
- **领域知识**：问题表述、硬约束与软约束、数值方法、精度、准确性和数值稳定性。理解这些基本规则能让开发人员在应用算法技术时更具创造性。

本书的目标是为所有这些领域提供坚实的基础。读者在读完本书后，应继续拓宽在这些领域的知识。最重要的是，培养更多计算思维技能的最佳方法是不断用出色的计算解决方案来解决具有挑战性的问题。有效利用计算的一个良好目标是让科学变得更好，而不仅仅是更快。这需要重新审视先前的假设，并切实思考如何应用大规模并行处理这一强大手段。换句话说，可能不会仅仅因为重新编译或在相同计算方法中使用更多线程就获得诺贝尔奖。真正重要的科学发现更可能来自全新的计算思维。考虑如何以新的方式利用这一强大的计算能力来解决新问题。

有三种方法可用于处理计算密集型应用程序，其难度、复杂性以及潜在回报依次递增。我们将它们分别称为 “良好”“更好” 和 “最佳” 方法。

“良好” 的方法仅仅是 “加速” 传统程序代码。最基本的做法就是在不添加任何领域见解或并行性专业知识的情况下，重新编译并在新平台或架构上运行代码。可以通过使用优化库、工具或指令（如 CuBLAS、CuFFT、Thrust、Matlab 或 OpenACC）来增强这种方法。此方法不需要任何算法选择、问题分解或优化与调优工作。对于领域科学家来说，它能立即带来回报，因为只需很少的计算机科学知识或编程技能就能获得不错的加速效果。然而，它并没有充分发挥并行计算的全部潜力。

“更好” 的方法涉及利用并行编程技能重写现有代码，以利用新架构的优势，或者从头创建新代码。这种方法为巧妙思考问题分解和优化选择提供了机会，对于非领域计算机科学家来说是不错的工作，因为所需的领域知识很少。然而，由于缺乏特定领域知识（这是有效选择算法所必需的），它也没有充分发挥并行计算的全部潜力。

“最佳” 方法是对应用程序并行化进行全面尝试，涉及三个关键步骤：算法选择、问题分解以及优化和调优。我们不仅希望将已知算法映射到并行程序并对其进行优化，还希望重新思考在解决方案中使用的数值方法和算法。第 18 章 “静电势图” 中的截断分箱方法就是一个很好的例子。这种方法需要领域专业知识，以便用准确性来换取大幅降低的算法复杂度，并且需要问题分解和优化技能，以便结合计算机科学家设计的分箱技术，采用基于网格点的分解方法。

在这种方法中，有可能获得最大的性能优势，以及实现根本性的新发现和新能力。例如，人们或许能够对一个生化系统进行高保真模拟，而该系统的规模被认为是传统方法无法企及的。这种方法具有跨学科性，既需要计算机科学知识，也需要领域见解，但回报值得付出努力。成为一名计算科学家，真的正处于一个激动人心的时代！

### 19.5 总结

总之，我们讨论了并行编程和计算思维的主要步骤，即算法选择、问题分解以及优化和调优。对于给定的计算问题，程序员通常必须从各种算法中进行选择。其中一些算法在保持相同数值精度的同时，实现了不同的权衡。另一些算法则涉及牺牲一定程度的精度，以实现更具可扩展性的运行时间。对于选定的算法，问题分解的不同选择可能会导致线程间干扰程度不同、展现出的并行性不同、负载不平衡，以及在并行执行期间的其他性能考量。计算思维技能使算法设计者能够绕过障碍，找到良好的解决方案。

# 20 异构计算集群编程 CUDA流的介绍

到目前为止，我们一直专注于对具有一个主机和一个设备的异构计算系统进行编程。在高性能计算（HPC）中，应用程序需要计算节点集群的聚合计算能力。如今，许多高性能计算集群在每个节点中都有一个或多个主机以及一个或多个设备。从历史上看，对这些集群进行编程主要使用消息传递接口（MPI）。在本章中，我们将介绍 MPI/CUDA 联合编程。我们将只介绍程序员在集群环境中把异构应用程序扩展到多个节点时需要理解的 MPI 概念。特别地，我们将重点关注在将 CUDA 内核扩展到多个节点的背景下的域划分、点对点通信和集合通信。
...
到目前为止，在异构计算系统中，我们假定了一种相当简单的主机与设备之间的交互模型。如第2章“异构数据并行计算”中所述，在这个简单模型里，每个设备都有一个与主机内存（即系统内存）分离的设备内存（CUDA全局内存）。要由设备上运行的内核处理的数据，需要通过调用`cudaMemcpy()`函数从主机内存传输到设备内存。设备产生的数据在被主机使用之前，也需要通过调用`cudaMemcpy()`函数从设备内存传输到主机。虽然这个模型简单易懂，但在应用层面会导致一些问题。 

首先，像磁盘控制器和网络接口卡这样的 I/O 设备被设计为在主机内存上高效运行。由于设备内存与主机内存是分离的，输入数据需要从主机内存传输到设备内存，输出数据需要从设备内存传输到主机内存，以便在 I/O 操作中使用。这种额外的传输增加了 I/O 延迟，并降低了可实现的 I/O 吞吐量。对于许多应用来说，I/O 设备能够直接在设备内存上运行将提高整体应用性能，并简化应用代码。

其次，主机内存是传统编程系统放置其应用数据结构的地方。有些数据结构很大。早期支持 CUDA 的 GPU 的设备内存与主机内存相比很小，这迫使应用开发人员将其大型数据结构划分为块，以便适合设备内存。例如，在第 18 章 “静电势图” 中，三维（3D）静电能网格数组被划分为二维切片，在主机内存和设备内存之间传输。对于许多应用来说，如果整个数据结构能够驻留在设备内存中，情况会好得多。对于某些应用，甚至将数据结构划分为更小的块也不是一个好方法。对于这些应用来说，如果 GPU 能够直接访问主机内存中的数据，或者如果 CUDA 运行时系统软件能够在需要时迁移在内核执行期间使用的数据，那将是最好的。

主机 / 设备交互模型的这些局限性源于早期支持 CUDA 的 GPU 的内存架构的局限性。在这些早期设备中，对于应用程序而言，唯一可行的主机 / 设备交互模型就是我们在前几章中所假定的简单模型。随着越来越多的应用采用 GPU 计算，它们的需求促使 CUDA 系统软件开发者和 GPU 硬件设计者提供更好的解决方案。自 CUDA 早期以来，研究人员就已经意识到这些需求并提出了解决方案（格拉多等人，2010 年）。本节的其余部分将简要回顾针对这些局限性的发展历程。

### 零拷贝内存与统一虚拟地址空间

2009年，CUDA 2.2引入了对系统内存的零拷贝访问功能。这使得主机代码能够向设备上运行的内核提供一个特殊的设备数据指针，以便访问主机内存。运行在内核中的代码可以使用此指针，通过系统互连（如PCIe总线）直接访问主机内存，而无需调用`cudaMemcpy()`函数。零拷贝内存是固定的主机内存（第20章，异构计算集群编程），通过调用`cudaHostAlloc()`函数并将`cudaHostAllocMapped`作为标志参数的值来分配。我们提到，该标志参数的其他值不能以更高级的方式使用。`cudaHostAlloc()`返回的数据指针不能直接传递给内核；主机代码必须首先使用`cudaHostGetDevicePointer()`获取一个有效的设备数据指针，并将此函数返回的设备数据指针传递给内核。这意味着主机代码和设备代码使用不同的数据指针来访问相同的物理内存。 

正如我们在第20章“异构计算集群编程”中所解释的，主机内存页必须被固定，以防止操作系统在GPU访问数据时意外地将数据换出。显然，这种访问会受到系统互连的长延迟和有限带宽的影响。系统互连的带宽通常不到全局内存带宽的10%。正如我们在第5章“内存架构与数据局部性”中所了解的，内核的性能通常受到全局内存带宽的限制，除非我们使用平铺技术大幅减少每次执行的浮点运算所对应的全局内存访问次数。如果内核的大多数内存访问都是零拷贝内存访问，那么内核的执行速度可能会更严重地受到系统互连带宽的限制。因此，应该仅对偶尔由GPU上运行的内核访问的应用数据结构使用零拷贝内存。 

2011年，CUDA 4引入了统一虚拟寻址（Unified Virtual Addressing）。在这个CUDA版本发布之前，主机和设备拥有各自的虚拟地址空间，它们各自将主机或设备数据指针映射到主机或设备的物理内存位置。这些不相交的虚拟地址空间意味着，相同的物理内存位置在主机和设备中可能通过不同的虚拟地址进行访问，这在使用零拷贝内存时确实会发生。统一虚拟地址空间（UVAS）由GMAC库首次引入（格拉多等人，2010年），并在CUDA 4中采用，它使用一个由主机和设备共享的单一虚拟地址空间。

 UVAS保证每个物理内存地址仅映射到一个虚拟内存位置。这使得CUDA运行时能够仅通过检查数据指针的地址来确定该指针是引用主机还是设备内存。此功能消除了在调用`cudaMemcpy()`时指定数据拷贝方向的需要。 

重要的是要注意，CUDA 4中的UVAS并不保证指针所引用数据的可访问性。例如，主机代码不能使用`cudaMalloc()`返回的设备指针直接访问设备内存，反之亦然。零拷贝内存是个例外：主机代码可以直接将指向零拷贝内存的指针作为内核启动参数传递给设备。当内核代码解引用这个零拷贝指针时，指针值会被转换为一个物理系统内存位置，并通过PCIe总线进行访问。请注意，这种方法不一定允许内核代码解引用从内存位置读取的指针值，例如在遍历链接数据结构时跟随指针链，除非所有内存都是使用`cudaHostAlloc()`分配的。 这两种数据结构类型在虚拟内存访问中的带宽限制，促使人们在UVAS之外对GPU架构的内存模型进行进一步的改进。 

### 大虚拟地址空间和大物理地址空间

早期支持 CUDA 的 GPU 的一个根本限制在于其虚拟地址和物理地址的大小。这些早期设备支持 32 位虚拟地址，且物理地址最多为 32 位。对于这些设备而言，设备内存大小被限制在 4GB，这是 32 位物理地址所能寻址的最大内存量。此外，CUDA 内核只能处理大小小于 4GB 的数据集，这是通过 32 位指针所能访问的虚拟内存位置的最大数量，无论数据集位于主机内存还是设备内存中。

再者，现代 CPU 基于 64 位虚拟地址，其中实际使用 48 位。这些主机虚拟地址无法被 GPU 所使用的 32 位虚拟地址所容纳，这也导致了零拷贝内存所支持的数据结构类型受到限制。

为了消除这一限制，从 2013 年推出的开普勒（Kepler）GPU 架构开始的几代 GPU，采用了具有 64 位虚拟地址和至少 40 位物理地址的现代虚拟内存架构。明显的好处是，这些 GPU 可以整合超过 4GB 的动态随机存取存储器（DRAM），并且 CUDA 内核现在可以处理大型数据集。虽然扩大后的虚拟地址空间和物理地址空间显然允许使用更大的设备内存，但它们也为更好的主机 / 设备交互模型打开了大门。例如，主机和设备现在可以使用完全相同的指针值来访问一块数据，而不管它是在主机内存还是设备内存中。

大的 GPU 物理地址空间还允许 CUDA 系统软件将系统中不同 GPU 的内存放置到一个统一的物理地址空间中。其好处是，一个 GPU 可以通过简单地解引用映射到其他 GPU 物理地址的数据指针，直接访问连接到同一 PCIe 总线的任何其他 GPU 的内存。在开普勒 GPU 架构之前，不同 GPU 之间的通信（例如，第 20 章 “异构计算集群编程” 中模板示例里的光晕交换）只能通过主机代码触发的设备到设备的内存拷贝来实现。这导致了存储从其他 GPU 拷贝来的数据所消耗的额外内存，以及由于内存拷贝操作而产生的额外性能开销。直接访问系统中其他设备的内存，使得在内核调用中只需传递指向其他 GPU 的设备指针，并使用它来加载和 / 或存储需要通信的数据。

### 统一内存

2013年，CUDA 6引入了统一内存，它创建了一个由CPU和GPU共享的托管内存池，弥合了CPU - GPU之间的鸿沟。托管内存可供CPU和GPU使用单个指针进行访问。托管内存中的变量可以驻留在CPU物理内存、GPU物理内存中，甚至可以同时存在于两者之中。CUDA运行时软件和硬件实现了数据迁移和一致性支持，例如GMAC系统（格拉多等人，2010年）。最终效果是，对于在CPU上运行的代码来说，托管内存看起来像CPU内存，而对于在GPU上运行的代码来说，它看起来像GPU内存。当然，应用程序必须执行适当的同步操作，如屏障或原子操作，以协调对托管内存位置的任何并发访问。共享的全局虚拟地址空间允许应用程序中的所有变量都具有唯一地址。当在编程工具和运行时系统中公开这种内存架构时，会带来几个主要好处。 

其中一个好处是将CPU代码移植到CUDA所需的工作量减少了。在图22.1中，我们在左侧展示了一个简单的CPU代码示例。使用统一内存，只需进行两个简单的更改就可以将代码移植到CUDA。第一个更改是使用`cudaMallocManaged()`和`cudaFree()`来代替`malloc()`和`free()`。第二个更改是启动一个内核并执行设备同步，而不是调用`qsort()`函数。显然，仍然需要编写或访问并行排序内核。我们展示的重点是，对主机代码的更改直接明了且易于维护。 

![image-20250208150019634](imgs/image-20250208150019634.png)

CUDA 6 统一内存的性能受到开普勒（Kepler）和麦克斯韦（Maxwell）GPU 架构的硬件能力限制。在启动任何网格（grid）之前，所有被 CPU 修改过的托管内存位置的内容都必须刷新到 GPU 设备内存中。CPU 和 GPU 不能同时访问托管内存，并且统一内存地址空间受限于 GPU 物理内存的大小。出现这些限制是因为这些 GPU 架构缺乏支持主机和设备内存之间一致性的能力，并且数据迁移大多由软件执行。

2016 年，帕斯卡（Pascal）GPU 架构添加了功能，进一步简化了在 CPU 和 GPU 之间进行内存编程和共享的过程，并减少了使用 GPU 实现显著加速所需的工作量。有两项主要硬件特性促成了这些改进：对大地址空间的支持和页面错误处理能力。

帕斯 卡GPU 架构将 GPU 的寻址能力扩展到 49 位虚拟寻址。这一扩展与 CPU 自身的 48 位虚拟地址空间一样大。现代 CPU 以及 GPU 的内存由此允许统一内存程序将系统中所有 CPU 和 GPU 的完整地址空间作为单个虚拟地址空间进行访问，而不受可复制到设备内存的数据量的限制。因此，CPU 和 GPU 可以真正共享指针值，使 GPU 能够遍历主机内存中的基于指针的数据结构。

帕斯卡 GPU 架构中的内存页面错误处理支持是一项关键的新特性，它提供了更无缝的统一内存功能。与系统范围的虚拟地址空间相结合，处理页面错误的能力消除了 CUDA 系统软件在每次启动网格（grid）之前同步（刷新）所有托管内存内容到 GPU 系统的需要。CUDA 运行时可以实现一种一致性机制，当主机和设备修改托管内存中的变量时，允许它们使彼此的副本无效。无效操作可以通过使用页面映射和保护机制来完成。当启动一个网格（grid）时，CUDA 系统软件不再需要将所有 GPU 的托管内存副本更新为最新。如果网格（grid）访问设备内存中其副本已无效的数据，GPU 将处理页面错误以使数据保持最新并继续执行。

如果在 GPU 上运行的网格（grid）访问其设备内存中不存在的页面，它也会产生页面错误，从而允许该页面根据需求自动迁移到 GPU 内存中。或者，如果预计数据只是偶尔被访问，该页面可以被映射到 GPU 地址空间，以便通过系统互连进行访问（按需映射有时可能比迁移更快）。请注意，统一内存是系统级的：GPU（和 CPU）可能会发生故障并从其他 GPU 或 CPU 内存中迁移内存页面。如果 CPU 函数解引用一个指针并访问映射到 GPU 物理内存的变量，数据访问仍然可以进行，但延迟可能会更长。这种能力使得 CUDA 程序能够更轻松地调用尚未移植到 GPU 的遗留库。在以前的 CUDA 内存架构中，开发人员必须手动将数据从设备内存传输到主机内存，以便使用遗留库函数在 CPU 上对其进行处理。

具有页面错误处理功能的统一内存实现了一种比零拷贝内存更通用的 GPU / 主机交互机制。它允许 GPU 遍历主机内存中的大型数据结构。从帕斯卡（Pascal）架构开始，即使数据结构不驻留在零拷贝内存中，GPU 设备也可以遍历链接数据结构。这是因为主机代码和设备代码使用相同的指针值来引用相同的变量。因此，主机构建的链接数据结构的嵌入式指针值可以被设备遍历，反之亦然。在一些应用领域，如计算机辅助设计（CAD），主机物理内存系统可能具有数百 GB 的容量。这些物理内存系统是必需的，因为应用程序需要整个数据集都 “在核心中”。有了直接访问非常大的 CPU 物理内存的能力，GPU 加速这些应用程序就变得可行了。

### 虚拟地址空间控制

 CUDA 11引入了一组底层API，为程序员在内存分配方面提供了更大的灵活性。新的API允许使用`cuMemAddressReserve()`预留一段虚拟地址空间。之后，程序员可以使用`cuMemCreate()`分配物理内存，并使用`cuMemMap()`将其映射到预留范围内的任何位置。这些API使得能够在多个设备上构建自定义的数据结构布局。例如，有可能在多个设备上分配一个三维体数据，同时使用单个指针来引用它。 

## 22.2 内核执行控制 

### 内核函数中的函数调用 

早期的CUDA版本不允许在内核执行期间进行函数调用。尽管内核函数的源代码可能看起来有函数调用，但编译器必须能够将所有函数体内联到内核对象中，以便在内核运行时没有函数调用。虽然这种模型在许多应用程序的性能关键部分运行良好，但它不支持更复杂应用程序中的常见软件工程实践。特别是，它不支持系统调用、动态链接库调用、递归函数调用以及C++等面向对象语言中的虚函数。 

后来的设备架构，如开普勒（Kepler），支持在内核函数运行时进行函数调用。此功能在CUDA 5及更高版本中得到支持。编译器不再需要将函数体进行内联，但它仍然可以作为一种性能优化手段来进行内联。这种能力部分得益于为CUDA线程实现的缓存式且快速的大规模并行调用栈。它使得CUDA设备代码更具“可组合性”，允许不同作者编写不同的CUDA内核组件并将它们组合在一起，而无需大量的重新设计成本。它还允许软件供应商在不提供源代码以保护知识产权的情况下发布设备库。然而，仍然存在一些限制。例如，对虚函数的支持仅限于由设备代码构造的对象，并且对设备代码的动态库也不支持。 

对运行时函数调用的支持也显著减轻了程序员从传统的面向CPU的算法过渡到针对GPU优化的代码时的负担。在某些情况下，开发人员将能够将CPU代码“剪切并粘贴”到CUDA内核中，并获得合理的性能，尽管性能调优仍然会带来好处。 

有了函数调用支持，内核现在可以调用标准库函数，如`printf()`和`malloc()`。根据我们的经验，在内核中调用`printf()`的能力在调试和支持生产软件中的内核方面提供了微妙但重要的帮助。许多终端用户是非技术人员，并且难以接受培训以运行调试器，而调试器本可以为开发人员提供有关崩溃前发生情况的更多细节。在内核中调用`printf()`的能力使开发人员能够向应用程序添加有意义的转储内部状态的代码，以便终端用户可以提交有用的错误报告。

 CUDA 8增加了对C++11的支持，随之而来的是另一种函数调用形式：lambda表达式。与元编程技术结合使用时，设备lambda表达式有助于开发高性能的可重用代码。CUDA还支持将lambda函数作为参数传递给CUDA内核。此功能可用于编写通用内核（例如，排序内核），其中比较函数只是内核的一个输入参数。CUDA还添加了对“扩展lambda表达式”的实验性支持，通过`--extended-lambda`编译器标志启用。此功能允许程序员使用`__host__`和`__device__`修饰符来注释C++ lambda表达式，进一步简化了编写可重用代码的任务。 

### 内核函数中的异常处理 

早期的CUDA系统不支持在内核代码中进行异常处理。虽然对于许多高性能应用程序的性能关键部分来说，这并不是一个重大限制，但在依赖异常来检测和处理罕见情况（而无需编写代码来显式测试这些情况）的生产质量应用程序中，它往往会产生软件工程成本。 由于有了有限的异常处理支持，CUDA调试器允许用户执行单步执行、设置断点，或者运行内核直到发生无效内存访问。在每种情况下，当执行暂停时，用户都可以检查内核局部变量和全局变量的值。根据我们的经验，CUDA调试器是检测内存越界访问和潜在竞态条件的非常有用的工具。 



### 多个网格的并发执行 

最早的CUDA系统在任何时刻只允许每个GPU设备上执行一个网格。可以使用CUDA流提交多个网格以进行执行，但它们会被缓冲在一个队列中，当前网格执行完成后才会释放下一个网格。费米（Fermi）GPU架构及其后续架构允许同时执行多个网格，这减轻了应用程序开发人员为了更充分地利用设备而将多个内核“批量”合并为一个更大内核的压力。此外，将工作划分为可以以不同优先级执行的块有时也是有益的。

 并发执行多个网格的好处的一个典型例子是并行集群应用程序，这些应用程序将工作划分为“本地”和“远程”部分，其中远程工作涉及与其他节点的交互，并且处于全局进展的关键路径上（第20章，异构计算集群编程）。在以前的CUDA系统中，网格需要执行大量工作以高效利用设备，并且必须小心不要启动本地工作，以免阻塞全局工作。这意味着在等待远程工作到达时选择不充分利用设备，或者急切地开始本地工作，以增加完成远程工作单元的延迟为代价来保持设备的高效性（菲利普斯和斯通，2009年）。通过多个网格执行，应用程序可以使用小得多的网格大小来启动工作，因此，当高优先级的远程工作到达时，它可以以低延迟开始运行，而不是被卡在大量本地计算的网格后面。 
